...
/**
 * Audit fields that are typically auto-generated by the backend
 * These are excluded from forms by default unless --include-audit-fields is used
 */
const AUDIT_FIELDS = [
  'id',
  'created_at',
  ...
  generateColumnHtml(column, entityVar, isFirst) {
    ...
    if (isFirst) {
      ...
      cellContent = `
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="text-sm font-semibold text-[var(--ax-text-default)]">
                      ${
                        isTruncated
                    ...
                    @if (${entityVar}.description && !isRowExpanded(${entityVar})) {
                      <div class="text-xs text-[var(--ax-text-secondary)] mt-0.5 truncate">
      ...
      cellContent = `
                @if (${entityVar}.${name}) {
                  <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-1 text-md font-medium text-[var(--ax-success-emphasis)] bg-[var(--ax-success-faint)] rounded"
                    ><span class="w-2 h-2 bg-[var(--ax-success-default)] rounded-full"></span
                    >Active</span
                  >
                } @else {
                  <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-1 text-md font-medium text-[var(--ax-text-secondary)] bg-[var(--ax-background-subtle)] rounded"
                    ><span class="w-2 h-2 bg-[var(--ax-text-disabled)] rounded-full"></span
                    >Inactive</span
                  >
                }`;
    } else if (isDate) {
...
`;
   ...
   * @param {Object} types - TypeScript type definitions
   * @param {string} entityName - Entity name in PascalCase
   * @param {boolean} isCreate - Whether generating for create dialog
   * @param {Object} enhancedSchema - Enhanced database schema info
   * @param {Object} options - Generation options (including includeAuditFields)
   */
  generateFormFields(
    types,
    entityName,
    ...
    const fields = [];
    const includeAuditFields = options.includeAuditFields || false;

    Object.keys(entityType).forEach((fieldName) => {
      // Skip audit fields by default (unless --include-audit-fields is used)
      if (!includeAuditFields && AUDIT_FIELDS.includes(fieldName)) {
        return;
      }
  ...
  getFormInputType(fieldName, fieldType, enhancedColumn = null) {
    // Use enhanced column info if available
    if (enhancedColumn) {
      // First check the raw database type for datetime columns
      const dbType = enhancedColumn.type?.toLowerCase() || '';
      if (dbType.includes('timestamp') || dbType === 'date') {
        return dbType === 'date' ? 'date' : 'datetime-local';
      }
      ...
      switch (enhancedColumn.fieldType) {
        ...
        case 'decimal':
          return 'number';
        default:
          break; // Continue to PostgreSQL type mapping
      }
    }
  ...
  async generateDialogComponents(moduleName, options = {}) {
    try {
      ...
      // Define number types for type coercion
      const numberTypes = [
        'number',
        'integer',
        'decimal',
      ...
      const sharedFormContext = {
        ...
        // JSON fields - for type coercion (string → object)
        hasJsonFields: sharedFormFields.some((field) => field.type === 'json'),
        jsonFields: sharedFormFields.filter((field) => field.type === 'json'),
        // Number fields - for type coercion (string → number)
        hasNumberFields: sharedFormFields.some((field) =>
          numberTypes.includes(field.type),
        ),
        numberFields: sharedFormFields.filter((field) =>
          numberTypes.includes(field.type),
        ),
        // DateTime fields
  ...
  /**
   * Auto-register frontend route in shell routes file (e.g., system.routes.ts)
   * This enables shell-based registration for proper integration with shell apps
   *
  ...
  async autoRegisterShellRoute(moduleName, shellName, options = {}) {
    ...
    const shellRoutesPath = path.join(
      this.projectRoot,
      `apps/${targetApp}/src/app/features/${shellKebab}/${shellKebab}.routes.ts`,
    );

    try {
      // Check if shell routes file exists
      if (!fs.existsSync(shellRoutesPath)) {
        console.warn(`⚠️ Shell routes file not found: ${shellRoutesPath}`);
      ...
      let content = fs.readFileSync(shellRoutesPath, 'utf8');

      // Check if already registered
      if (content.includes(`path: '${kebabName}'`)) {
        console.log(
          `⚠️ ${moduleName} route already registered in ${shellName}.routes.ts`,
        );
        return false;
      }
      ...
      // Write back
      fs.writeFileSync(shellRoutesPath, content);

      console.log(
        `✅ Auto-registered ${moduleName} route in ${shellName} shell:`,
      );
      console.log(`   - App: ${targetApp}`);
      ...
      console.error(
        `❌ Failed to auto-register route in ${shellName} shell:`,
        error.message,
      );
      console.log(
        `💡 Please register manually in apps/${targetApp}/src/app/features/${shellKebab}/${shellKebab}.routes.ts`,
      );
      return false;
    }
  }
