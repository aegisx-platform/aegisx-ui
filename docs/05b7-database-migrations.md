      ...
      switch (role.name) {
        ...
        case 'admin':
          ...
          allowedPermissions = permissions
            .filter(p => p.resource !== 'system' || p.action === 'read')
            .map(p => p.id);
          break;
        ...
        case 'manager':
          ...
          allowedPermissions = permissions
            .filter(p => 
              ['users', 'products', 'orders', 'customers', 'reports'].includes(p.resource) &&
    ...
    for (let i = 0; i < userCount; i++) {
      ...
      testUsers.push({
        ...
        timezone: faker.helpers.arrayElement([
          'UTC', 'America/New_York', 'America/Los_Angeles',
          'Europe/London', 'Asia/Bangkok', 'Asia/Tokyo'
        ]),
        language: 'en',
...
export class AuditLogsSeed extends BaseSeed {
  async seed(): Promise<void> {
    ...
    const actions = [
      ...
      'GET /api/reports/users', 'GET /api/reports/activities',
      'POST /api/auth/login', 'POST /api/auth/logout',
      'PUT /api/users/:id/profile', 'POST /api/users/:id/change-password'
    ];
...
export class EnvironmentSpecificSeed extends BaseSeed {
  ...
  private async seedProductionData(): Promise<void> {
    ...
    const adminCount = await this.knex('users')
    ...
    if (parseInt(adminCount?.count as string) === 0) {
      ...
      const superAdminRole = await this.knex('roles')
  ...
  static async create(options: UserFactoryOptions = {}) {
    ...
    if (includeProfile) {
      return {
        ...
        timezone: faker.helpers.arrayElement([
          'UTC', 'America/New_York', 'Europe/London', 'Asia/Bangkok'
        ]),
        language: 'en'
      };
    }
...
export class MigrationUtils {
  ...
  static async addFullTextSearch(
    knex: Knex, 
    tableName: string, 
    searchColumn: string, 
  ...
  static async createPartitionedTable(
    knex: Knex,
    tableName: string,
    partitionKey: string,
...
export class SeedReset {
  ...
  private async runSeeds(): Promise<void> {
    ...
    const seedFiles = [
      '../seeds/01_roles',
      '../seeds/02_permissions', 
      '../seeds/03_role_permissions',
      '../seeds/04_users',
      '../seeds/05_audit_logs'
    ];
