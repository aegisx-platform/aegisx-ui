...
const { getEnhancedSchema, validateDropdownEndpoints } = require('./database.js');
...
class DependencyValidator {
  constructor() {
    this.apiDir = path.resolve(__dirname, '..', '..', '..', 'apps', 'api', 'src');
    this.webDir = path.resolve(__dirname, '..', '..', '..', 'apps', 'web', 'src');
  }
    ...
    try {
      ...
      for (const column of enhancedSchema.columns) {
        if (column.fieldType === 'foreign-key-dropdown' && column.dropdownInfo) {
          ...
          const endpointExists = await this.checkEndpointExists(referencedTable, 'dropdown');
  ...
  async checkEndpointExists(moduleName, endpointType = 'dropdown') {
    try {
      // Check routes file
      const routesPath = path.join(this.apiDir, 'modules', moduleName, 'routes', 'index.ts');
      if (!fs.existsSync(routesPath)) {
        // Try core directory
        const coreRoutesPath = path.join(this.apiDir, 'core', moduleName, `${moduleName}.routes.ts`);
  ...
  async validateFrontendDependencies(moduleName) {
    ...
    try {
      // Check service file
      const servicePath = path.join(this.webDir, 'app', 'features', moduleName, 'services', `${moduleName}.service.ts`);
      results.services.exists = fs.existsSync(servicePath);
      
      if (results.services.exists) {
        const serviceContent = fs.readFileSync(servicePath, 'utf8');
        results.services.hasDropdownMethods = serviceContent.includes('dropdown');
      ...
      // Check types file
      const typesPath = path.join(this.webDir, 'app', 'features', moduleName, 'types', `${moduleName}.types.ts`);
      const altTypesPath = path.join(this.webDir, 'app', 'features', moduleName, 'types', `${moduleName.slice(0, -1)}.types.ts`);
      
      results.types.exists = fs.existsSync(typesPath) || fs.existsSync(altTypesPath);
      
      if (results.types.exists) {
        const typeContent = fs.readFileSync(fs.existsSync(typesPath) ? typesPath : altTypesPath, 'utf8');
      ...
      const componentsDir = path.join(this.webDir, 'app', 'features', moduleName, 'components');
      if (fs.existsSync(componentsDir)) {
        const files = fs.readdirSync(componentsDir);
        results.components.createDialog = files.some(f => f.includes('create') && f.includes('dialog'));
        results.components.editDialog = files.some(f => f.includes('edit') && f.includes('dialog'));
        results.components.viewDialog = files.some(f => f.includes('view') && f.includes('dialog'));
        results.components.listComponent = files.some(f => f.includes('list') && f.includes('component'));
      }
  ...
  async validateApiFrontendAlignment(moduleName) {
    ...
    try {
      ...
      if (fs.existsSync(servicePath)) {
        ...
        for (const column of enhancedSchema.columns) {
          if (column.fieldType === 'foreign-key-dropdown' && column.dropdownInfo) {
            const dropdownServicePattern = new RegExp(`${column.foreignKeyInfo.referencedTable}.*dropdown`, 'i');
  ...
  async generateValidationReport(moduleName) {
    ...
    const [dropdownValidation, frontendValidation, alignmentValidation] = await Promise.all([
    ...
    const report = {
      ...
      summary: {
        ...
        overallStatus: dropdownValidation.valid && frontendValidation.valid && alignmentValidation.aligned ? 'PASS' : 'FAIL'
      },
      details: {
  ...
  displayReport(report) {
    ...
    console.log(`\n🔍 Summary:`);
    console.log(`  Dropdown Dependencies: ${report.summary.dropdownsValid ? '✅' : '❌'}`);
    console.log(`  Frontend Complete: ${report.summary.frontendComplete ? '✅' : '❌'}`);
    ...
    if (report.details.dropdowns.availableEndpoints.length > 0) {
      ...
      report.details.dropdowns.availableEndpoints.forEach(endpoint => {
        ...
        if (endpoint.exists) {
          console.log(`      Display fields: ${endpoint.displayFields.join(', ')}`);
        }
      });
    }
...
if (require.main === module) {
  ...
  validator.generateValidationReport(moduleName)
    .then(report => {
