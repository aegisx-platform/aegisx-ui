...
async function validateModule(moduleName, options = {}) {
  ...
  // Check test file
  const testFilePath = path.join(modulePath, '__tests__', `${moduleName}.test.ts`);
  try {
  ...
  try {
    ...
    if (!controllerContent.includes('class') || !controllerContent.includes('Controller')) {
      errors.push('Controller class not found or improperly named');
    }
    
    if (!controllerContent.includes('create') || !controllerContent.includes('findOne')) {
  ...
  try {
    ...
    if (!serviceContent.includes('BaseRepository')) {
      warnings.push('Service does not extend BaseRepository - may not follow AegisX patterns');
    }
...
async function validateConventions(moduleName, options = {}) {
  ...
  try {
    ...
    const expectedPattern = new RegExp(`^${moduleName}\\.(controller|service|routes|schemas|types|plugin)\\.ts$`);
    
    files.forEach(file => {
      if (file.endsWith('.ts') && !file.includes('test') && !expectedPattern.test(file)) {
