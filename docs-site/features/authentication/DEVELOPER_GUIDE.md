...
```
apps/api/src/core/authentication/
├── controllers/
...
```
apps/web/src/app/features/authentication/
├── components/
...
export const CreateFeatureSchema = Type.Omit(FeatureSchema, ['id', 'createdAt', 'updatedAt']);
...
export async function featureRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/', {
    ...
    preValidation: [fastify.authenticate, fastify.verifyPermission('features', 'create')],
...
```typescript
// Backend - emit event
this.eventService.for('features', 'features').emitCustom('created', feature, 'normal');
