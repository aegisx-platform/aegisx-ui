...
import {
  UserRealtimeStateService,
  User,
} from '../../business/services/user-realtime-state.service';
    ...
    <mat-card appearance="outlined" class="realtime-user-list">
      <mat-card-header>
        <mat-card-title class="flex items-center gap-2">
          ...
          <mat-chip-set>
            <mat-chip [class.selected]="userStateService.isConnected()">
              <mat-icon>{{
                userStateService.isConnected() ? 'wifi' : 'wifi_off'
              }}</mat-icon>
              {{
        ...
        <mat-card-subtitle>
          ...
          <span
            *ngIf="userStateService.hasConflicts()"
      ...
      <mat-card-content>
        ...
        <div class="controls-row mb-4 flex gap-2 flex-wrap">
          ...
          <button
            *ngIf="userStateService.hasPendingOperations()"
            mat-stroked-button
            color="warn"
            (click)="forcePush()"
            [disabled]="userStateService.isLoading()"
          >
            <mat-icon
              [matBadge]="userStateService.pendingOperations().length"
              matBadgeColor="warn"
              >upload</mat-icon
            >
            Push Changes
          ...
          >
            <mat-icon
              [matBadge]="userStateService.conflicts().size"
              matBadgeColor="warn"
              >warning</mat-icon
            >
            Resolve Conflicts
        ...
        <!-- Loading Indicator -->
        <div
          *ngIf="userStateService.isLoading()"
          class="loading-container text-center py-4"
        >
        ...
        <!-- User List -->
        <div
          class="user-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
        >
          <div
          ...
          >
            ...
            <div class="user-header flex items-center justify-between mb-2">
              <div class="user-info flex items-center gap-2">
                <div
                  class="avatar w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm"
                >
              ...
              <div class="status-indicators flex gap-1">
                <mat-chip *ngIf="!user.isActive" class="status-warn"
                  >Inactive</mat-chip
                >
                ...
                <mat-chip
                  *ngIf="hasPendingOperation(user.id)"
                  class="status-accent"
                >
        ...
        <!-- Empty State -->
        <div
          *ngIf="displayedUsers().length === 0 && !userStateService.isLoading()"
          class="empty-state text-center py-8"
        >
          <mat-icon class="text-6xl text-gray-400 mb-2"
...
export class RealtimeUserListComponent implements OnInit {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (pendingCount > 0) {
        this.snackBar.open(`${pendingCount} operations pending sync`, 'Push', {
          duration: 3000,
          panelClass: 'accent',
        });
      }
  ...
  public async syncData(): Promise<void> {
    try {
      ...
      this.snackBar.open('Sync failed. Please try again.', 'OK', {
        duration: 3000,
      });
    }
  }
  ...
  public async addTestUser(): Promise<void> {
    ...
    try {
      ...
      this.snackBar.open('✅ Test user created (Optimistic)', 'OK', {
        duration: 2000,
      });
    } catch (error: any) {
  ...
  public async addTestUserServerOnly(): Promise<void> {
    ...
    try {
      ...
      this.snackBar.open('✅ Test user created (Server-only)', 'OK', {
        duration: 2000,
      });
    } catch (error: any) {
      console.error('Failed to create test user:', error);
  ...
  public editUser(user: User): void {
    ...
    this.snackBar.open('Edit functionality coming soon', 'OK', {
      duration: 2000,
    });
  }
  ...
  public showConflictDetails(userId: string): void {
    ...
    if (conflict) {
      ...
      this.snackBar.open('Conflict resolution dialog coming soon', 'OK', {
        duration: 2000,
      });
    }
  }
