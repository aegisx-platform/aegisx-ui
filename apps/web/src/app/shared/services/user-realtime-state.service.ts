...
import { BaseRealtimeStateManager, BaseEntity, StateOptions, ConflictInfo } from './base-realtime-state-manager';
...
export class UserRealtimeStateService extends BaseRealtimeStateManager<User> {
  ...
  private handleHttpError = (error: any) => {
    ...
    if (error.status === 403) {
      ...
      throw new Error('ไม่มีสิทธิ์ในการดำเนินการนี้ (Insufficient permissions)');
  ...
  protected async fetchFromServer(): Promise<User[]> {
    try {
      ...
      const response = await firstValueFrom(
        this.http.get<{ success: boolean; data: User[] }>(url, { 
  ...
  protected async serverCreate(data: Omit<User, 'id'>): Promise<User> {
    try {
      ...
      const response = await firstValueFrom(
        this.http.post<{ success: boolean; data: User }>(url, userData, {
          headers: this.getHttpHeaders()
        }).pipe(
          catchError(this.handleHttpError)
        )
  ...
  protected async serverUpdate(id: string | number, changes: Partial<User>): Promise<User> {
    try {
      const url = `${this.apiConfig.getApiBaseUrl()}/api/users/${id}`;
      const response = await firstValueFrom(
        this.http.put<{ success: boolean; data: User }>(url, changes, {
          headers: this.getHttpHeaders()
        }).pipe(
          catchError(this.handleHttpError)
        )
  ...
  protected async serverDelete(id: string | number): Promise<void> {
    try {
      console.log('🗑️ UserRealtimeStateService.serverDelete called for ID:', id);
      const url = `${this.apiConfig.getApiBaseUrl()}/api/users/${id}`;
  ...
  public async createUser(userData: CreateUserRequest, options?: { skipOptimistic?: boolean }): Promise<User> {
  ...
  public async updateUser(id: string, changes: UpdateUserRequest): Promise<User> {
  ...
  public searchUsers(query: string): User[] {
    ...
    return this.localState().filter(user => 
  ...
  private isMinorConflict(conflict: ConflictInfo): boolean {
    ...
    return conflict.conflictedFields.every(field => minorFields.includes(field));
  }
