...
export class ActivityLogService {
  ...
  readonly hasNextPage = computed(() => this.paginationSignal()?.hasNext ?? false);
  readonly hasPrevPage = computed(() => this.paginationSignal()?.hasPrev ?? false);
  ...
  loadActivities(filters?: ActivityLogFilters): Observable<ActivityLogResponse> {
    ...
    if (currentFilters.page) httpParams = httpParams.set('page', currentFilters.page.toString());
    if (currentFilters.limit) httpParams = httpParams.set('limit', currentFilters.limit.toString());
    if (currentFilters.action) httpParams = httpParams.set('action', currentFilters.action);
    if (currentFilters.severity) httpParams = httpParams.set('severity', currentFilters.severity);
    if (currentFilters.search) httpParams = httpParams.set('search', currentFilters.search);
    if (currentFilters.dateFrom) httpParams = httpParams.set('dateFrom', currentFilters.dateFrom);
    if (currentFilters.dateTo) httpParams = httpParams.set('dateTo', currentFilters.dateTo);
   ...
   */
  loadStats(): Observable<ActivityLogStats> {
    return this.http.get<ApiResponse<ActivityLogStats>>(`${this.baseUrl}/stats`).pipe(
      map(response => {
        if (!response.success || !response.data) {
      ...
      catchError(error => {
        const errorMessage = error.error?.error || error.message || 'Failed to load stats';
        this.errorSignal.set(errorMessage);
        throw error;
      })
    );
