...
@Component({
  selector: 'app-test-products-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
...
export class TestProductImportDialogComponent implements OnDestroy {
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.testProductsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `test-products-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.testProductsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.testProductsService.executeImport(
