...
import { CreateBudgetAllocationRequest } from '../types/budget-allocations.types';
import {
...
@Component({
  selector: 'app-budget-allocations-create-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    ...
    <h2 mat-dialog-title class="ax-header ax-header-info">
      ...
      <div class="header-text">
        <div class="ax-title">Create New BudgetAllocation</div>
        <div class="ax-subtitle">
          Add a new budgetallocation to your collection
        </div>
      </div>
      <button
        type="button"
...
export class BudgetAllocationCreateDialogComponent {
  private budgetAllocationsService = inject(BudgetAllocationService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(
    MatDialogRef<BudgetAllocationCreateDialogComponent>,
  );
  ...
  async onFormSubmit(formData: BudgetAllocationFormData) {
    ...
    try {
      ...
      const createRequest =
        formData as unknown as CreateBudgetAllocationRequest;
      const result =
        await this.budgetAllocationsService.createBudgetAllocation(
          createRequest,
        );

      if (result) {
        this.snackBar.open('BudgetAllocation created successfully', 'Close', {
          duration: 3000,
        });
        this.dialogRef.close(result);
      } else {
        this.snackBar.open('Failed to create budgetallocation', 'Close', {
          duration: 5000,
        });
      }
    } catch (error: any) {
      const errorMessage = this.budgetAllocationsService.permissionError()
        ? 'You do not have permission to create budgetallocation'
        : error?.message || 'Failed to create budgetallocation';
