...
@Component({
  selector: 'app-budget-allocations-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Budget Allocations</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="budget_id">
                      <th mat-header-cell *matHeaderCellDef>Budget Id</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.budget_id }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Budget Allocations...</h3>
...
export class BudgetAllocationImportDialogComponent implements OnDestroy {
  private budgetAllocationsService = inject(BudgetAllocationService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(
    MatDialogRef<BudgetAllocationImportDialogComponent>,
  );
    ...
    this.budgetAllocationsService
      ...
      .subscribe({
        next: (blob) => {
          ...
          link.href = url;
          link.download = `budget-allocations-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
          link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.budgetAllocationsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.budgetAllocationsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.budgetAllocationsService.getImportStatus(jobId);
