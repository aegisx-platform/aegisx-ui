...
@Component({
  selector: 'app-companies-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Companies</h2>
...
export class CompanieImportDialogComponent implements OnDestroy {
  private companiesService = inject(CompanieService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<CompanieImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.companiesService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `companies-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.companiesService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.companiesService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.companiesService.getImportStatus(jobId);
