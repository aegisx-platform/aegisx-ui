...
@Component({
  selector: 'app-budgets-edit-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    ...
    <h2 mat-dialog-title class="ax-header ax-header-warning">
      ...
      <div class="header-text">
        <div class="ax-title">Edit Budget</div>
        <div class="ax-subtitle">Update budget information</div>
      </div>
      <button
        type="button"
        mat-icon-button
...
export class BudgetEditDialogComponent implements OnInit {
  ...
  async onFormSubmit(formData: BudgetFormData) {
    ...
    try {
      ...
      const updateRequest = formData as unknown as UpdateBudgetRequest;
