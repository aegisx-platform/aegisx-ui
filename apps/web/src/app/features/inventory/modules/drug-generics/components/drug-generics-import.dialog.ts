...
@Component({
  selector: 'app-drug-generics-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Drug Generics</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="working_code">
                      <th mat-header-cell *matHeaderCellDef>Working Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.working_code }}
                      </td>
                    </ng-container>
...
export class DrugGenericImportDialogComponent implements OnDestroy {
  private drugGenericsService = inject(DrugGenericService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DrugGenericImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.drugGenericsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `drug-generics-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.drugGenericsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.drugGenericsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.drugGenericsService.getImportStatus(jobId);
