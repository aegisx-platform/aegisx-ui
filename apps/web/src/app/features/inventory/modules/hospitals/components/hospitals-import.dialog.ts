...
@Component({
  selector: 'app-hospitals-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Hospitals</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <!-- Data Columns -->
                    <ng-container matColumnDef="hospital_code">
                      <th mat-header-cell *matHeaderCellDef>Hospital Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.hospital_code }}
                      </td>
                    </ng-container>
                    ...
                    <ng-container matColumnDef="hospital_type">
                      <th mat-header-cell *matHeaderCellDef>Hospital Type</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.hospital_type }}
                      </td>
                    </ng-container>
...
export class HospitalImportDialogComponent implements OnDestroy {
  private hospitalsService = inject(HospitalService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<HospitalImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.hospitalsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `hospitals-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.hospitalsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.hospitalsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.hospitalsService.getImportStatus(jobId);
  ...
  get hasFailed(): boolean {
    ...
    return !!(job?.failedCount && job.failedCount > 0);
  }
