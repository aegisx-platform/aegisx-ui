...
@Component({
  selector: 'app-dosage-forms-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Dosage Forms</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <!-- Data Columns -->
                    <ng-container matColumnDef="form_code">
                    ...
                    <ng-container matColumnDef="form_name_en">
                      <th mat-header-cell *matHeaderCellDef>Form Name En</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.form_name_en }}
                      </td>
                    </ng-container>
...
export class DosageFormImportDialogComponent implements OnDestroy {
  private dosageFormsService = inject(DosageFormService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DosageFormImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.dosageFormsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `dosage-forms-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.dosageFormsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.dosageFormsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.dosageFormsService.getImportStatus(jobId);
