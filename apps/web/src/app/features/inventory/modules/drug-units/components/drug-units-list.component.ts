...
import { DrugUnit, ListDrugUnitQuery } from '../types/drug-units.types';
import { DrugUnitCreateDialogComponent } from './drug-units-create.dialog';
import {
  DrugUnitEditDialogComponent,
  DrugUnitEditDialogData,
} from './drug-units-edit.dialog';
import {
  DrugUnitViewDialogComponent,
  DrugUnitViewDialogData,
} from './drug-units-view.dialog';
import { DrugUnitImportDialogComponent } from './drug-units-import.dialog';
import {
...
import { DrugUnitsListFiltersComponent } from './drug-units-list-filters.component';
import { DrugUnitsListHeaderComponent } from './drug-units-list-header.component';

@Component({
  selector: 'app-drug-units-list',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
    ...
    BreadcrumbComponent,
    // Child components
    DrugUnitsListHeaderComponent,
    DrugUnitsListFiltersComponent,
    SharedExportComponent,
  ...
  templateUrl: './drug-units-list.component.html',
  styleUrl: './drug-units-list.component.scss',
  animations: [
...
export class DrugUnitsListComponent {
  drugUnitsService = inject(DrugUnitService);
  ...
  dataSource = new MatTableDataSource<DrugUnit>([]);
  selection = new SelectionModel<DrugUnit>(true, []);

  // Selection for export feature (like authors)
  private selectedIdsSignal = signal<Set<number>>(new Set());
  readonly selectedItems = computed(() =>
    this.drugUnitsService
      .drugUnitsList()
  ...
  protected unit_codeInputSignal = signal('');
  protected unit_nameInputSignal = signal('');
  protected unit_name_enInputSignal = signal('');
  ...
  protected unit_codeFilterSignal = signal('');
  protected unit_nameFilterSignal = signal('');
  protected unit_name_enFilterSignal = signal('');
  ...
  protected expandedDrugUnit = signal<DrugUnit | null>(null);

  // Computed signals
  advancedFilters = computed(() => ({
    unit_code: this.unit_codeInputSignal(),
    unit_name: this.unit_nameInputSignal(),
    unit_name_en: this.unit_name_enInputSignal(),
  ...
  get unit_codeFilter() {
    return this.unit_codeInputSignal();
  }
  set unit_codeFilter(value: string) {
    this.unit_codeInputSignal.set(value);
  }
  get unit_nameFilter() {
    return this.unit_nameInputSignal();
  }
  set unit_nameFilter(value: string) {
    this.unit_nameInputSignal.set(value);
  }
  get unit_name_enFilter() {
    return this.unit_name_enInputSignal();
  }
  set unit_name_enFilter(value: string) {
    this.unit_name_enInputSignal.set(value);
  }
  ...
  stats = computed(() => {
    const list = this.drugUnitsService.drugUnitsList();
    const total = this.drugUnitsService.totalDrugUnit();
  ...
  exportServiceAdapter: ExportService = {
    export: (options: ExportOptions) =>
      this.drugUnitsService.exportDrugUnit(options),
  };
  ...
  availableExportFields = [
    ...
    { key: 'unit_name_en', label: 'Unit Name En' },
  ...
  // --- Effect: reload drug_units on sort/page/search/filter change ---
  constructor() {
    ...
    effect(() => {
      const loading = this.drugUnitsService.loading();
    ...
    effect(async () => {
      ...
      const unit_code = this.unit_codeFilterSignal();
      const unit_name = this.unit_nameFilterSignal();
      const unit_name_en = this.unit_name_enFilterSignal();
      const description = this.descriptionFilterSignal();
      const is_active = this.is_activeFilterSignal();

      const params: Partial<ListDrugUnitQuery> = {
        page: (page?.index ?? 0) + 1,
        limit: page?.size ?? 25,
        sort:
        ...
        unit_code: unit_code?.trim() || undefined,
        unit_name: unit_name?.trim() || undefined,
        unit_name_en: unit_name_en?.trim() || undefined,
      ...
      await this.drugUnitsService.loadDrugUnitList(params);
      this.dataSource.data = this.drugUnitsService.drugUnitsList();
      if (this.paginator) {
        this.paginator.length = this.drugUnitsService.totalDrugUnit();
      }
    });
  }
  ...
  refresh() {
    ...
    this.unit_codeInputSignal.set('');
    this.unit_codeFilterSignal.set('');
    this.unit_nameInputSignal.set('');
    this.unit_nameFilterSignal.set('');
    this.unit_name_enInputSignal.set('');
    this.unit_name_enFilterSignal.set('');
  ...
  applyFilterImmediate() {
    ...
    this.unit_codeFilterSignal.set(this.unit_codeInputSignal().trim());
    this.unit_nameFilterSignal.set(this.unit_nameInputSignal().trim());
    this.unit_name_enFilterSignal.set(this.unit_name_enInputSignal().trim());
  ...
  hasActiveFilters(): boolean {
    return (
      ...
      this.unit_codeFilterSignal().trim() !== '' ||
      this.unit_nameFilterSignal().trim() !== '' ||
      this.unit_name_enFilterSignal().trim() !== '' ||
  ...
  getActiveFilterCount(): number {
    ...
    if (this.unit_codeFilterSignal().trim()) count++;
    if (this.unit_nameFilterSignal().trim()) count++;
    if (this.unit_name_enFilterSignal().trim()) count++;
  ...
  // CRUD Operations
  openCreateDialog() {
    const dialogRef = this.dialog.open(DrugUnitCreateDialogComponent, {
      width: '600px',
    });
  ...
  openImportDialog() {
    const dialogRef = this.dialog.open(DrugUnitImportDialogComponent, {
      width: '900px',
      maxHeight: '90vh',
    });
  ...
  onViewDrugUnit(drugUnit: DrugUnit) {
    const dialogRef = this.dialog.open(DrugUnitViewDialogComponent, {
      width: '600px',
      data: { drugUnits: drugUnit } as DrugUnitViewDialogData,
    });

    dialogRef.afterClosed().subscribe((result: any) => {
      if (result?.action === 'edit') {
        this.onEditDrugUnit(result.data);
      }
    });
  }

  onEditDrugUnit(drugUnit: DrugUnit) {
    const dialogRef = this.dialog.open(DrugUnitEditDialogComponent, {
      width: '600px',
      data: { drugUnits: drugUnit } as DrugUnitEditDialogData,
    });
  ...
  onDeleteDrugUnit(drugUnit: DrugUnit) {
    const itemName =
      (drugUnit as any).name || (drugUnit as any).title || 'drugunit';
    this.axDialog.confirmDelete(itemName).subscribe(async (confirmed) => {
      if (confirmed) {
        try {
          await this.drugUnitsService.deleteDrugUnit(drugUnit.id);
          this.snackBar.open('DrugUnit deleted successfully', 'Close', {
            duration: 3000,
          });
          this.reloadTrigger.update((n) => n + 1);
        } catch {
          this.snackBar.open('Failed to delete drugunit', 'Close', {
            duration: 3000,
          });
        }
      }
  ...
  bulkDelete() {
    ...
    this.axDialog
      .confirmBulkDelete(count, 'drug_units')
      .subscribe(async (confirmed) => {
        if (confirmed) {
          try {
            const deletePromises = this.selection.selected.map((drugUnit) =>
              this.drugUnitsService.deleteDrugUnit(drugUnit.id),
            );

            await Promise.all(deletePromises);
            this.snackBar.open(
              `${count} drugunit(s) deleted successfully`,
              'Close',
              { duration: 3000 },
            );
            this.selection.clear();
            ...
            this.snackBar.open('Failed to delete some drug_units', 'Close', {
              duration: 3000,
            });
          }
        }
  ...
  getExportFilters(): Record<string, unknown> {
    return {
      ...
      unit_code: this.unit_codeFilterSignal(),
      unit_name: this.unit_nameFilterSignal(),
      unit_name_en: this.unit_name_enFilterSignal(),
  ...
  toggleExpandRow(drugUnit: DrugUnit): void {
    const currentExpanded = this.expandedDrugUnit();
    if (currentExpanded?.id === drugUnit.id) {
      // Collapse currently expanded row
      this.expandedDrugUnit.set(null);
    } else {
      // Expand new row (and collapse previous if any)
      this.expandedDrugUnit.set(drugUnit);
    }
  }

  isRowExpanded(drugUnit: DrugUnit): boolean {
    return this.expandedDrugUnit()?.id === drugUnit.id;
  }
}
