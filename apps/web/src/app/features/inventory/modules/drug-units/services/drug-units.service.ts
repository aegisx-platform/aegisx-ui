...
export class DrugUnitService {
  private http = inject(HttpClient);
  private baseUrl = '/inventory/master-data/drug-units';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private drugUnitsListSignal = signal<DrugUnit[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedDrugUnitSignal = signal<DrugUnit | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalDrugUnitSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly drugUnitsList = this.drugUnitsListSignal.asReadonly();
  ...
  readonly selectedDrugUnit = this.selectedDrugUnitSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalDrugUnit = this.totalDrugUnitSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalDrugUnitSignal();
  ...
  /**
   * Load drugUnits list with pagination and filters
   */
  async loadDrugUnitList(params?: ListDrugUnitQuery): Promise<void> {
    ...
    try {
      ...
      // String filtering for unit_code
      if (params?.unit_code)
        httpParams = httpParams.set('unit_code', params.unit_code);
      // String filtering for unit_name
      if (params?.unit_name)
        httpParams = httpParams.set('unit_name', params.unit_name);
      // String filtering for unit_name_en
      if (params?.unit_name_en)
        httpParams = httpParams.set('unit_name_en', params.unit_name_en);
      ...
      const response = await this.http
        .get<PaginatedResponse<DrugUnit>>(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.drugUnitsListSignal.set(response.data);

        if (response.pagination) {
          this.totalDrugUnitSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugUnits list');
    } finally {
  ...
  /**
   * Load single drugUnits by ID
   */
  async loadDrugUnitById(id: number): Promise<DrugUnit | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<DrugUnit>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedDrugUnitSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugUnits');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createDrugUnit(data: CreateDrugUnitRequest): Promise<DrugUnit | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<DrugUnit>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create drugUnits');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async updateDrugUnit(
    id: number,
    data: UpdateDrugUnitRequest,
  ): Promise<DrugUnit | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<DrugUnit>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update drugUnits');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete drugUnits by ID
   */
  async deleteDrugUnit(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete drugUnits');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export drugUnits data
   */
  async exportDrugUnit(options: {
    ...
    try {
      ...
      console.error('Failed to export drugUnits data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for drugUnits
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch drugUnits dropdown options:', error);
      return [];
    }
  }

  /**
   * Bulk create drugUnitss
   */
  async bulkCreateDrugUnit(
    items: CreateDrugUnitRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadDrugUnitList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create drugUnitss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update drugUnitss
   */
  async bulkUpdateDrugUnit(
    items: Array<{ id: number; data: UpdateDrugUnitRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update drugUnitss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete drugUnitss
   */
  async bulkDeleteDrugUnit(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete drugUnitss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate drugUnits data before save
   */
  async validateDrugUnit(
    data: CreateDrugUnitRequest,
    ...
    try {
      ...
      console.error('Failed to validate drugUnits:', error);
  ...
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get drugUnits stats:', error);
      return null;
    }
  }
   ...
   * Select drugUnits
   */
  selectDrugUnit(drugUnits: DrugUnit | null): void {
    this.selectedDrugUnitSignal.set(drugUnits);
  }
   ...
   */
  reset(): void {
    this.drugUnitsListSignal.set([]);
    this.selectedDrugUnitSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalDrugUnitSignal.set(0);
  }
}
