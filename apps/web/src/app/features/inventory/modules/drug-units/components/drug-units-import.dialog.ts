...
@Component({
  selector: 'app-drug-units-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Drug Units</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="unit_name_en">
                      <th mat-header-cell *matHeaderCellDef>Unit Name En</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.unit_name_en }}
                      </td>
                    </ng-container>
...
export class DrugUnitImportDialogComponent implements OnDestroy {
  private drugUnitsService = inject(DrugUnitService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DrugUnitImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.drugUnitsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `drug-units-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.drugUnitsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.drugUnitsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.drugUnitsService.getImportStatus(jobId);
