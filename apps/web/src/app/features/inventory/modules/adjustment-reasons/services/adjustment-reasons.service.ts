...
export class AdjustmentReasonService {
  ...
  private baseUrl = '/inventory/master-data/adjustment-reasons';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private adjustmentReasonsListSignal = signal<AdjustmentReason[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedAdjustmentReasonSignal = signal<AdjustmentReason | null>(
    null,
  );
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalAdjustmentReasonSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly adjustmentReasonsList =
    this.adjustmentReasonsListSignal.asReadonly();
  ...
  readonly selectedAdjustmentReason =
    this.selectedAdjustmentReasonSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalAdjustmentReason =
    this.totalAdjustmentReasonSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalAdjustmentReasonSignal();
  ...
  /**
   * Load adjustmentReasons list with pagination and filters
   */
  async loadAdjustmentReasonList(
    params?: ListAdjustmentReasonQuery,
  ): Promise<void> {
    ...
    try {
      ...
      // String filtering for reason_code
      if (params?.reason_code)
        httpParams = httpParams.set('reason_code', params.reason_code);
      // String filtering for reason_name
      if (params?.reason_name)
        httpParams = httpParams.set('reason_name', params.reason_name);
      // Boolean filtering for requires_approval
      if (params?.requires_approval !== undefined)
        httpParams = httpParams.set(
          'requires_approval',
          params.requires_approval.toString(),
        );
      ...
      if (response) {
        this.adjustmentReasonsListSignal.set(response.data);

        if (response.pagination) {
          this.totalAdjustmentReasonSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load adjustmentReasons list');
    } finally {
  ...
  /**
   * Load single adjustmentReasons by ID
   */
  async loadAdjustmentReasonById(id: number): Promise<AdjustmentReason | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<AdjustmentReason>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedAdjustmentReasonSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load adjustmentReasons');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async createAdjustmentReason(
    data: CreateAdjustmentReasonRequest,
  ): Promise<AdjustmentReason | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<AdjustmentReason>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create adjustmentReasons');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async updateAdjustmentReason(
    id: number,
    data: UpdateAdjustmentReasonRequest,
  ): Promise<AdjustmentReason | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<AdjustmentReason>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update adjustmentReasons');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete adjustmentReasons by ID
   */
  async deleteAdjustmentReason(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete adjustmentReasons');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export adjustmentReasons data
   */
  async exportAdjustmentReason(options: {
    ...
    try {
      ...
      console.error('Failed to export adjustmentReasons data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for adjustmentReasons
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error(
        'Failed to fetch adjustmentReasons dropdown options:',
        error,
      );
      return [];
    }
  ...
  /**
   * Bulk create adjustmentReasonss
   */
  async bulkCreateAdjustmentReason(
    items: CreateAdjustmentReasonRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadAdjustmentReasonList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create adjustmentReasonss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update adjustmentReasonss
   */
  async bulkUpdateAdjustmentReason(
    items: Array<{ id: number; data: UpdateAdjustmentReasonRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update adjustmentReasonss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete adjustmentReasonss
   */
  async bulkDeleteAdjustmentReason(
    ids: number[],
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete adjustmentReasonss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate adjustmentReasons data before save
   */
  async validateAdjustmentReason(
    data: CreateAdjustmentReasonRequest,
    ...
    try {
      ...
      console.error('Failed to validate adjustmentReasons:', error);
  ...
  /**
   * Get adjustmentReasons statistics
   */
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get adjustmentReasons stats:', error);
      return null;
    }
  }
  ...
  /**
   * Select adjustmentReasons
   */
  selectAdjustmentReason(adjustmentReasons: AdjustmentReason | null): void {
    this.selectedAdjustmentReasonSignal.set(adjustmentReasons);
  }
   ...
   */
  reset(): void {
    this.adjustmentReasonsListSignal.set([]);
    this.selectedAdjustmentReasonSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalAdjustmentReasonSignal.set(0);
  }
}
