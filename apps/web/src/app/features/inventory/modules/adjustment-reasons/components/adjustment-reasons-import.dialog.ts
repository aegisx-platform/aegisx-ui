...
@Component({
  selector: 'app-adjustment-reasons-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Adjustment Reasons</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="reason_name">
                      <th mat-header-cell *matHeaderCellDef>Reason Name</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.reason_name }}
                      </td>
                    </ng-container>
                    <ng-container matColumnDef="adjustment_type">
                      <th mat-header-cell *matHeaderCellDef>Adjustment Type</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.adjustment_type }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Adjustment Reasons...</h3>
...
export class AdjustmentReasonImportDialogComponent implements OnDestroy {
  private adjustmentReasonsService = inject(AdjustmentReasonService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(
    MatDialogRef<AdjustmentReasonImportDialogComponent>,
  );
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.adjustmentReasonsService
      .downloadImportTemplate(format, true)
      .subscribe({
        next: (blob) => {
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.download = `adjustment-reasons-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
          link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.adjustmentReasonsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.adjustmentReasonsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.adjustmentReasonsService.getImportStatus(jobId);
