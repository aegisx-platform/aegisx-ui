...
export class DrugComponentService {
  private http = inject(HttpClient);
  private baseUrl = '/inventory/master-data/drug-components';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private drugComponentsListSignal = signal<DrugComponent[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedDrugComponentSignal = signal<DrugComponent | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalDrugComponentSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly drugComponentsList = this.drugComponentsListSignal.asReadonly();
  ...
  readonly selectedDrugComponent =
    this.selectedDrugComponentSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalDrugComponent = this.totalDrugComponentSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalDrugComponentSignal();
  ...
  /**
   * Load drugComponents list with pagination and filters
   */
  async loadDrugComponentList(params?: ListDrugComponentQuery): Promise<void> {
    ...
    try {
      ...
      // String filtering for component_name
      if (params?.component_name)
        httpParams = httpParams.set('component_name', params.component_name);
      // String filtering for strength
      if (params?.strength)
        httpParams = httpParams.set('strength', params.strength);
      ...
      const response = await this.http
        .get<
          PaginatedResponse<DrugComponent>
        >(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.drugComponentsListSignal.set(response.data);

        if (response.pagination) {
          this.totalDrugComponentSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugComponents list');
    } finally {
  ...
  /**
   * Load single drugComponents by ID
   */
  async loadDrugComponentById(id: number): Promise<DrugComponent | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<DrugComponent>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedDrugComponentSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugComponents');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createDrugComponent(
    data: CreateDrugComponentRequest,
  ): Promise<DrugComponent | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<DrugComponent>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create drugComponents');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async updateDrugComponent(
    id: number,
    data: UpdateDrugComponentRequest,
  ): Promise<DrugComponent | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<DrugComponent>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update drugComponents');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete drugComponents by ID
   */
  async deleteDrugComponent(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete drugComponents');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export drugComponents data
   */
  async exportDrugComponent(options: {
    ...
    try {
      ...
      console.error('Failed to export drugComponents data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for drugComponents
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch drugComponents dropdown options:', error);
      return [];
    }
  }
  ...
  /**
   * Bulk create drugComponentss
   */
  async bulkCreateDrugComponent(
    items: CreateDrugComponentRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadDrugComponentList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create drugComponentss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update drugComponentss
   */
  async bulkUpdateDrugComponent(
    items: Array<{ id: number; data: UpdateDrugComponentRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update drugComponentss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete drugComponentss
   */
  async bulkDeleteDrugComponent(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete drugComponentss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate drugComponents data before save
   */
  async validateDrugComponent(
    data: CreateDrugComponentRequest,
    ...
    try {
      ...
      console.error('Failed to validate drugComponents:', error);
  ...
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get drugComponents stats:', error);
      return null;
    }
  }
  ...
  /**
   * Select drugComponents
   */
  selectDrugComponent(drugComponents: DrugComponent | null): void {
    this.selectedDrugComponentSignal.set(drugComponents);
  }
   ...
   */
  reset(): void {
    this.drugComponentsListSignal.set([]);
    this.selectedDrugComponentSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalDrugComponentSignal.set(0);
  }
}
