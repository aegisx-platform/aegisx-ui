...
@Component({
  selector: 'app-drug-components-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Drug Components</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="generic_id">
                      <th mat-header-cell *matHeaderCellDef>Generic Id</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.generic_id }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Drug Components...</h3>
...
export class DrugComponentImportDialogComponent implements OnDestroy {
  private drugComponentsService = inject(DrugComponentService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DrugComponentImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.drugComponentsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `drug-components-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.drugComponentsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.drugComponentsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.drugComponentsService.getImportStatus(jobId);
