...
export class ContractService {
  ...
  private baseUrl = '/inventory/master-data/contracts';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private contractsListSignal = signal<Contract[]>([]);
  ...
  private totalContractSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly contractsList = this.contractsListSignal.asReadonly();
  ...
  readonly totalContract = this.totalContractSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalContractSignal();
  ...
  /**
   * Load contracts list with pagination and filters
   */
  async loadContractList(params?: ListContractQuery): Promise<void> {
    ...
    try {
      ...
      if (params?.contract_number)
        httpParams = httpParams.set('contract_number', params.contract_number);
      // Numeric filtering for vendor_id
      if (params?.vendor_id !== undefined)
        httpParams = httpParams.set('vendor_id', params.vendor_id.toString());
      if (params?.vendor_id_min !== undefined)
        httpParams = httpParams.set(
          'vendor_id_min',
          params.vendor_id_min.toString(),
        );
      if (params?.vendor_id_max !== undefined)
        httpParams = httpParams.set(
          'vendor_id_max',
          params.vendor_id_max.toString(),
        );
      // Date/DateTime filtering for start_date
      if (params?.start_date)
        httpParams = httpParams.set('start_date', params.start_date);
      if (params?.start_date_min)
        httpParams = httpParams.set('start_date_min', params.start_date_min);
      if (params?.start_date_max)
        httpParams = httpParams.set('start_date_max', params.start_date_max);
      // Date/DateTime filtering for end_date
      if (params?.end_date)
        httpParams = httpParams.set('end_date', params.end_date);
      if (params?.end_date_min)
        httpParams = httpParams.set('end_date_min', params.end_date_min);
      if (params?.end_date_max)
        httpParams = httpParams.set('end_date_max', params.end_date_max);
      // Numeric filtering for total_value
      if (params?.total_value !== undefined)
        httpParams = httpParams.set(
          'total_value',
          params.total_value.toString(),
        );
      if (params?.total_value_min !== undefined)
        httpParams = httpParams.set(
          'total_value_min',
          params.total_value_min.toString(),
        );
      if (params?.total_value_max !== undefined)
        httpParams = httpParams.set(
          'total_value_max',
          params.total_value_max.toString(),
        );
      // Numeric filtering for remaining_value
      if (params?.remaining_value !== undefined)
        httpParams = httpParams.set(
          'remaining_value',
          params.remaining_value.toString(),
        );
      if (params?.remaining_value_min !== undefined)
        httpParams = httpParams.set(
          'remaining_value_min',
          params.remaining_value_min.toString(),
        );
      if (params?.remaining_value_max !== undefined)
        httpParams = httpParams.set(
          'remaining_value_max',
          params.remaining_value_max.toString(),
        );
      // String filtering for fiscal_year
      if (params?.fiscal_year)
        httpParams = httpParams.set('fiscal_year', params.fiscal_year);
      // String filtering for egp_number
      if (params?.egp_number)
        httpParams = httpParams.set('egp_number', params.egp_number);
      // String filtering for project_number
      if (params?.project_number)
        httpParams = httpParams.set('project_number', params.project_number);
      ...
      const response = await this.http
        .get<PaginatedResponse<Contract>>(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.contractsListSignal.set(response.data);

        if (response.pagination) {
          this.totalContractSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load contracts list');
    } finally {
  ...
  /**
   * Load single contracts by ID
   */
  async loadContractById(id: number): Promise<Contract | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<Contract>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedContractSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load contracts');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createContract(data: CreateContractRequest): Promise<Contract | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<Contract>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create contracts');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async updateContract(
    ...
    try {
      const response = await this.http
        .put<ApiResponse<Contract>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update contracts');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async deleteContract(id: number): Promise<boolean> {
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete contracts');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Get dropdown options for contracts
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch contracts dropdown options:', error);
      return [];
    }
  }

  /**
   * Get companies dropdown options for vendor_id field
   */
  ...
  /**
   * Bulk create contractss
   */
  async bulkCreateContract(
    items: CreateContractRequest[],
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk create contractss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update contractss
   */
  async bulkUpdateContract(
    items: Array<{ id: number; data: UpdateContractRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update contractss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete contractss
   */
  async bulkDeleteContract(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete contractss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   * Select contracts
   */
  selectContract(contracts: Contract | null): void {
    this.selectedContractSignal.set(contracts);
  }
   ...
   */
  reset(): void {
    this.contractsListSignal.set([]);
    this.selectedContractSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalContractSignal.set(0);
  }
}
