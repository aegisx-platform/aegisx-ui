...
export class DrugFocusListService {
  private http = inject(HttpClient);
  private baseUrl = '/inventory/master-data/drug-focus-lists';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private drugFocusListsListSignal = signal<DrugFocusList[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedDrugFocusListSignal = signal<DrugFocusList | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalDrugFocusListSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly drugFocusListsList = this.drugFocusListsListSignal.asReadonly();
  ...
  readonly selectedDrugFocusList =
    this.selectedDrugFocusListSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalDrugFocusList = this.totalDrugFocusListSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalDrugFocusListSignal();
  ...
  /**
   * Load drugFocusLists list with pagination and filters
   */
  async loadDrugFocusListList(params?: ListDrugFocusListQuery): Promise<void> {
    ...
    try {
      ...
      // String filtering for list_code
      if (params?.list_code)
        httpParams = httpParams.set('list_code', params.list_code);
      // String filtering for list_name
      if (params?.list_name)
        httpParams = httpParams.set('list_name', params.list_name);
      ...
      const response = await this.http
        .get<
          PaginatedResponse<DrugFocusList>
        >(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.drugFocusListsListSignal.set(response.data);

        if (response.pagination) {
          this.totalDrugFocusListSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugFocusLists list');
    } finally {
  ...
  /**
   * Load single drugFocusLists by ID
   */
  async loadDrugFocusListById(id: number): Promise<DrugFocusList | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<DrugFocusList>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedDrugFocusListSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load drugFocusLists');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createDrugFocusList(
    data: CreateDrugFocusListRequest,
  ): Promise<DrugFocusList | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<DrugFocusList>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create drugFocusLists');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async updateDrugFocusList(
    id: number,
    data: UpdateDrugFocusListRequest,
  ): Promise<DrugFocusList | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<DrugFocusList>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update drugFocusLists');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete drugFocusLists by ID
   */
  async deleteDrugFocusList(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete drugFocusLists');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export drugFocusLists data
   */
  async exportDrugFocusList(options: {
    ...
    try {
      ...
      console.error('Failed to export drugFocusLists data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for drugFocusLists
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch drugFocusLists dropdown options:', error);
      return [];
    }
  }
  ...
  /**
   * Bulk create drugFocusListss
   */
  async bulkCreateDrugFocusList(
    items: CreateDrugFocusListRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadDrugFocusListList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create drugFocusListss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update drugFocusListss
   */
  async bulkUpdateDrugFocusList(
    items: Array<{ id: number; data: UpdateDrugFocusListRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update drugFocusListss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete drugFocusListss
   */
  async bulkDeleteDrugFocusList(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete drugFocusListss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate drugFocusLists data before save
   */
  async validateDrugFocusList(
    data: CreateDrugFocusListRequest,
    ...
    try {
      ...
      console.error('Failed to validate drugFocusLists:', error);
  ...
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get drugFocusLists stats:', error);
      return null;
    }
  }
  ...
  /**
   * Select drugFocusLists
   */
  selectDrugFocusList(drugFocusLists: DrugFocusList | null): void {
    this.selectedDrugFocusListSignal.set(drugFocusLists);
  }
   ...
   */
  reset(): void {
    this.drugFocusListsListSignal.set([]);
    this.selectedDrugFocusListSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalDrugFocusListSignal.set(0);
  }
}
