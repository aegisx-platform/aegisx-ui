...
@Component({
  selector: 'app-drug-focus-lists-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Drug Focus Lists</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="list_code">
                      <th mat-header-cell *matHeaderCellDef>List Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.list_code }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Drug Focus Lists...</h3>
...
export class DrugFocusListImportDialogComponent implements OnDestroy {
  private drugFocusListsService = inject(DrugFocusListService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DrugFocusListImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.drugFocusListsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `drug-focus-lists-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.drugFocusListsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.drugFocusListsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.drugFocusListsService.getImportStatus(jobId);
