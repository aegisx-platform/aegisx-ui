...
import { CreateBudgetReservationRequest } from '../types/budget-reservations.types';
import {
...
@Component({
  selector: 'app-budget-reservations-create-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    ...
    <h2 mat-dialog-title class="ax-header ax-header-info">
      ...
      <div class="header-text">
        <div class="ax-title">Create New BudgetReservation</div>
        <div class="ax-subtitle">
          Add a new budgetreservation to your collection
        </div>
      </div>
      <button
        type="button"
...
export class BudgetReservationCreateDialogComponent {
  private budgetReservationsService = inject(BudgetReservationService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(
    MatDialogRef<BudgetReservationCreateDialogComponent>,
  );
  ...
  async onFormSubmit(formData: BudgetReservationFormData) {
    ...
    try {
      ...
      const createRequest =
        formData as unknown as CreateBudgetReservationRequest;
      const result =
        await this.budgetReservationsService.createBudgetReservation(
          createRequest,
        );

      if (result) {
        this.snackBar.open('BudgetReservation created successfully', 'Close', {
          duration: 3000,
        });
        this.dialogRef.close(result);
      } else {
        this.snackBar.open('Failed to create budgetreservation', 'Close', {
          duration: 5000,
        });
      }
    } catch (error: any) {
      const errorMessage = this.budgetReservationsService.permissionError()
        ? 'You do not have permission to create budgetreservation'
        : error?.message || 'Failed to create budgetreservation';
