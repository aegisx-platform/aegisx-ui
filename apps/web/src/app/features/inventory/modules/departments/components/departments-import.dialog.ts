...
@Component({
  selector: 'app-departments-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Departments</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="his_code">
                      <th mat-header-cell *matHeaderCellDef>His Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.his_code }}
                      </td>
                    </ng-container>
...
export class DepartmentImportDialogComponent implements OnDestroy {
  private departmentsService = inject(DepartmentService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DepartmentImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.departmentsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `departments-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.departmentsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.departmentsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.departmentsService.getImportStatus(jobId);
