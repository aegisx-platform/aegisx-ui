...
@Component({
  selector: 'app-budget-categories-list-filters',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
        ...
        <div class="mt-4 pt-4 border-t border-[var(--ax-border-default)]">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <mat-form-field appearance="outline" class="w-full">
              ...
              <input
                matInput
                [(ngModel)]="category_codeFilter"
                (ngModelChange)="category_codeFilterChange.emit($event)"
            ...
            <mat-form-field appearance="outline" class="w-full">
              ...
              <input
                matInput
                [(ngModel)]="category_nameFilter"
                (ngModelChange)="category_nameFilterChange.emit($event)"
                (keyup.enter)="applyFiltersClicked.emit()"
                placeholder="Enter category name"
              />
            </mat-form-field>
            <!-- Accounting Code Filter -->
            <mat-form-field appearance="outline" class="w-full">
              <mat-label>Accounting Code</mat-label>
              <input
                matInput
                [(ngModel)]="accounting_codeFilter"
                (ngModelChange)="accounting_codeFilterChange.emit($event)"
                (keyup.enter)="applyFiltersClicked.emit()"
...
export class BudgetCategoriesListFiltersComponent {
  ...
  // Category Code filter
  @Input() category_codeFilter = '';
  @Output() category_codeFilterChange = new EventEmitter<string>();
  // Category Name filter
  @Input() category_nameFilter = '';
  @Output() category_nameFilterChange = new EventEmitter<string>();
  // Accounting Code filter
  @Input() accounting_codeFilter = '';
  @Output() accounting_codeFilterChange = new EventEmitter<string>();
  // Description filter
