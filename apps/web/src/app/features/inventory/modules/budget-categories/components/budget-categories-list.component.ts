...
import {
  BudgetCategorie,
  ListBudgetCategorieQuery,
} from '../types/budget-categories.types';
import { BudgetCategorieCreateDialogComponent } from './budget-categories-create.dialog';
import {
  BudgetCategorieEditDialogComponent,
  BudgetCategorieEditDialogData,
} from './budget-categories-edit.dialog';
import {
  BudgetCategorieViewDialogComponent,
  BudgetCategorieViewDialogData,
} from './budget-categories-view.dialog';
import { BudgetCategorieImportDialogComponent } from './budget-categories-import.dialog';
import {
...
import { BudgetCategoriesListFiltersComponent } from './budget-categories-list-filters.component';
import { BudgetCategoriesListHeaderComponent } from './budget-categories-list-header.component';
...
@Component({
  ...
  imports: [
    ...
    BreadcrumbComponent,
    // Child components
    BudgetCategoriesListHeaderComponent,
    BudgetCategoriesListFiltersComponent,
    SharedExportComponent,
  ...
  styleUrl: './budget-categories-list.component.scss',
  animations: [
...
export class BudgetCategoriesListComponent {
  budgetCategoriesService = inject(BudgetCategorieService);
  ...
  dataSource = new MatTableDataSource<BudgetCategorie>([]);
  selection = new SelectionModel<BudgetCategorie>(true, []);

  // Selection for export feature (like authors)
  private selectedIdsSignal = signal<Set<number>>(new Set());
  readonly selectedItems = computed(() =>
    this.budgetCategoriesService
  ...
  protected category_codeInputSignal = signal('');
  protected category_nameInputSignal = signal('');
  protected accounting_codeInputSignal = signal('');
  ...
  protected category_codeFilterSignal = signal('');
  protected category_nameFilterSignal = signal('');
  protected accounting_codeFilterSignal = signal('');
  ...
  protected expandedBudgetCategorie = signal<BudgetCategorie | null>(null);

  // Computed signals
  advancedFilters = computed(() => ({
    category_code: this.category_codeInputSignal(),
    category_name: this.category_nameInputSignal(),
    accounting_code: this.accounting_codeInputSignal(),
  ...
  get category_codeFilter() {
    return this.category_codeInputSignal();
  }
  set category_codeFilter(value: string) {
    this.category_codeInputSignal.set(value);
  }
  get category_nameFilter() {
    return this.category_nameInputSignal();
  }
  set category_nameFilter(value: string) {
    this.category_nameInputSignal.set(value);
  }
  get accounting_codeFilter() {
    return this.accounting_codeInputSignal();
  }
  set accounting_codeFilter(value: string) {
    this.accounting_codeInputSignal.set(value);
  }
  ...
  stats = computed(() => {
    const list = this.budgetCategoriesService.budgetCategoriesList();
    const total = this.budgetCategoriesService.totalBudgetCategorie();
  ...
  exportServiceAdapter: ExportService = {
    export: (options: ExportOptions) =>
      this.budgetCategoriesService.exportBudgetCategorie(options),
  };
  ...
  // --- Effect: reload budget_categories on sort/page/search/filter change ---
  constructor() {
    ...
    effect(() => {
      const loading = this.budgetCategoriesService.loading();
    ...
    effect(async () => {
      ...
      const category_code = this.category_codeFilterSignal();
      const category_name = this.category_nameFilterSignal();
      const accounting_code = this.accounting_codeFilterSignal();
      const description = this.descriptionFilterSignal();
      const is_active = this.is_activeFilterSignal();

      const params: Partial<ListBudgetCategorieQuery> = {
        page: (page?.index ?? 0) + 1,
        limit: page?.size ?? 25,
        sort:
        ...
        category_code: category_code?.trim() || undefined,
        category_name: category_name?.trim() || undefined,
        accounting_code: accounting_code?.trim() || undefined,
      ...
      await this.budgetCategoriesService.loadBudgetCategorieList(params);
      this.dataSource.data =
        this.budgetCategoriesService.budgetCategoriesList();
      if (this.paginator) {
        this.paginator.length =
          this.budgetCategoriesService.totalBudgetCategorie();
      }
    });
  }
  ...
  refresh() {
    ...
    this.category_codeInputSignal.set('');
    this.category_codeFilterSignal.set('');
    this.category_nameInputSignal.set('');
    this.category_nameFilterSignal.set('');
    this.accounting_codeInputSignal.set('');
    this.accounting_codeFilterSignal.set('');
  ...
  applyFilterImmediate() {
    ...
    this.category_codeFilterSignal.set(this.category_codeInputSignal().trim());
    this.category_nameFilterSignal.set(this.category_nameInputSignal().trim());
    this.accounting_codeFilterSignal.set(
      this.accounting_codeInputSignal().trim(),
    );
  ...
  hasActiveFilters(): boolean {
    return (
      ...
      this.category_codeFilterSignal().trim() !== '' ||
      this.category_nameFilterSignal().trim() !== '' ||
      this.accounting_codeFilterSignal().trim() !== '' ||
  ...
  getActiveFilterCount(): number {
    ...
    if (this.category_codeFilterSignal().trim()) count++;
    if (this.category_nameFilterSignal().trim()) count++;
    if (this.accounting_codeFilterSignal().trim()) count++;
  ...
  // CRUD Operations
  openCreateDialog() {
    const dialogRef = this.dialog.open(BudgetCategorieCreateDialogComponent, {
      width: '600px',
    });
  ...
  openImportDialog() {
    const dialogRef = this.dialog.open(BudgetCategorieImportDialogComponent, {
      width: '900px',
      maxHeight: '90vh',
    });
  ...
  onViewBudgetCategorie(budgetCategorie: BudgetCategorie) {
    const dialogRef = this.dialog.open(BudgetCategorieViewDialogComponent, {
      width: '600px',
      data: {
        budgetCategories: budgetCategorie,
      } as BudgetCategorieViewDialogData,
    });

    dialogRef.afterClosed().subscribe((result: any) => {
      if (result?.action === 'edit') {
        this.onEditBudgetCategorie(result.data);
      }
    });
  }

  onEditBudgetCategorie(budgetCategorie: BudgetCategorie) {
    const dialogRef = this.dialog.open(BudgetCategorieEditDialogComponent, {
      width: '600px',
      data: {
        budgetCategories: budgetCategorie,
      } as BudgetCategorieEditDialogData,
    });
  ...
  onDeleteBudgetCategorie(budgetCategorie: BudgetCategorie) {
    const itemName =
      (budgetCategorie as any).name ||
      (budgetCategorie as any).title ||
      'budgetcategorie';
    this.axDialog.confirmDelete(itemName).subscribe(async (confirmed) => {
      if (confirmed) {
        try {
          await this.budgetCategoriesService.deleteBudgetCategorie(
            budgetCategorie.id,
          );
          this.snackBar.open('BudgetCategorie deleted successfully', 'Close', {
            duration: 3000,
          });
          this.reloadTrigger.update((n) => n + 1);
        } catch {
          this.snackBar.open('Failed to delete budgetcategorie', 'Close', {
            duration: 3000,
          });
        }
      }
  ...
  bulkDelete() {
    ...
    this.axDialog
      .confirmBulkDelete(count, 'budget_categories')
      .subscribe(async (confirmed) => {
        if (confirmed) {
          try {
            const deletePromises = this.selection.selected.map(
              (budgetCategorie) =>
                this.budgetCategoriesService.deleteBudgetCategorie(
                  budgetCategorie.id,
                ),
            );
            ...
            this.snackBar.open(
              `${count} budgetcategorie(s) deleted successfully`,
              'Close',
              { duration: 3000 },
            );
            this.selection.clear();
            ...
            this.snackBar.open(
              'Failed to delete some budget_categories',
              'Close',
              {
                duration: 3000,
              },
  ...
  getExportFilters(): Record<string, unknown> {
    return {
      ...
      category_code: this.category_codeFilterSignal(),
      category_name: this.category_nameFilterSignal(),
      accounting_code: this.accounting_codeFilterSignal(),
  ...
  toggleExpandRow(budgetCategorie: BudgetCategorie): void {
    const currentExpanded = this.expandedBudgetCategorie();
    if (currentExpanded?.id === budgetCategorie.id) {
      // Collapse currently expanded row
      this.expandedBudgetCategorie.set(null);
    } else {
      // Expand new row (and collapse previous if any)
      this.expandedBudgetCategorie.set(budgetCategorie);
    }
  }

  isRowExpanded(budgetCategorie: BudgetCategorie): boolean {
    return this.expandedBudgetCategorie()?.id === budgetCategorie.id;
  }
}
