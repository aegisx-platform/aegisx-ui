...
@Component({
  selector: 'app-budget-categories-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Budget Categories</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="accounting_code">
                      <th mat-header-cell *matHeaderCellDef>Accounting Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.accounting_code }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Budget Categories...</h3>
...
export class BudgetCategorieImportDialogComponent implements OnDestroy {
  private budgetCategoriesService = inject(BudgetCategorieService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(
    MatDialogRef<BudgetCategorieImportDialogComponent>,
  );
    ...
    this.budgetCategoriesService
      ...
      .subscribe({
        next: (blob) => {
          ...
          link.href = url;
          link.download = `budget-categories-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
          link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response =
        await this.budgetCategoriesService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.budgetCategoriesService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response =
          await this.budgetCategoriesService.getImportStatus(jobId);
