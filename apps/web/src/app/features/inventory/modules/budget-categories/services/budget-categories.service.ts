...
export class BudgetCategorieService {
  private http = inject(HttpClient);
  private baseUrl = '/inventory/master-data/budget-categories';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private budgetCategoriesListSignal = signal<BudgetCategorie[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedBudgetCategorieSignal = signal<BudgetCategorie | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalBudgetCategorieSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly budgetCategoriesList = this.budgetCategoriesListSignal.asReadonly();
  ...
  readonly selectedBudgetCategorie =
    this.selectedBudgetCategorieSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalBudgetCategorie = this.totalBudgetCategorieSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalBudgetCategorieSignal();
  ...
  /**
   * Load budgetCategories list with pagination and filters
   */
  async loadBudgetCategorieList(
    params?: ListBudgetCategorieQuery,
  ): Promise<void> {
    ...
    try {
      ...
      // String filtering for category_code
      if (params?.category_code)
        httpParams = httpParams.set('category_code', params.category_code);
      // String filtering for category_name
      if (params?.category_name)
        httpParams = httpParams.set('category_name', params.category_name);
      // String filtering for accounting_code
      if (params?.accounting_code)
        httpParams = httpParams.set('accounting_code', params.accounting_code);
      ...
      const response = await this.http
        .get<
          PaginatedResponse<BudgetCategorie>
        >(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.budgetCategoriesListSignal.set(response.data);

        if (response.pagination) {
          this.totalBudgetCategorieSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load budgetCategories list');
    } finally {
  ...
  /**
   * Load single budgetCategories by ID
   */
  async loadBudgetCategorieById(id: number): Promise<BudgetCategorie | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<BudgetCategorie>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedBudgetCategorieSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load budgetCategories');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createBudgetCategorie(
    data: CreateBudgetCategorieRequest,
  ): Promise<BudgetCategorie | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<BudgetCategorie>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create budgetCategories');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async updateBudgetCategorie(
    id: number,
    data: UpdateBudgetCategorieRequest,
  ): Promise<BudgetCategorie | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<BudgetCategorie>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update budgetCategories');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete budgetCategories by ID
   */
  async deleteBudgetCategorie(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete budgetCategories');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export budgetCategories data
   */
  async exportBudgetCategorie(options: {
    ...
    try {
      ...
      console.error('Failed to export budgetCategories data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for budgetCategories
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error(
        'Failed to fetch budgetCategories dropdown options:',
        error,
      );
      return [];
    }
  ...
  /**
   * Bulk create budgetCategoriess
   */
  async bulkCreateBudgetCategorie(
    items: CreateBudgetCategorieRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadBudgetCategorieList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create budgetCategoriess');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update budgetCategoriess
   */
  async bulkUpdateBudgetCategorie(
    items: Array<{ id: number; data: UpdateBudgetCategorieRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update budgetCategoriess');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete budgetCategoriess
   */
  async bulkDeleteBudgetCategorie(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete budgetCategoriess');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate budgetCategories data before save
   */
  async validateBudgetCategorie(
    data: CreateBudgetCategorieRequest,
    ...
    try {
      ...
      console.error('Failed to validate budgetCategories:', error);
  ...
  /**
   * Get budgetCategories statistics
   */
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get budgetCategories stats:', error);
      return null;
    }
  }
   ...
   */
  selectBudgetCategorie(budgetCategories: BudgetCategorie | null): void {
    this.selectedBudgetCategorieSignal.set(budgetCategories);
  }
   ...
   */
  reset(): void {
    this.budgetCategoriesListSignal.set([]);
    this.selectedBudgetCategorieSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalBudgetCategorieSignal.set(0);
  }
}
