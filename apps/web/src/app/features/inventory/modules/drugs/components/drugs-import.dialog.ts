...
@Component({
  selector: 'app-drugs-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Drugs</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <!-- Data Columns -->
                    <ng-container matColumnDef="drug_code">
...
export class DrugImportDialogComponent implements OnDestroy {
  private drugsService = inject(DrugService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<DrugImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.drugsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `drugs-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.drugsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.drugsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.drugsService.getImportStatus(jobId);
