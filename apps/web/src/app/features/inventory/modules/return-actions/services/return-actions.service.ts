...
export class ReturnActionService {
  ...
  private baseUrl = '/inventory/master-data/return-actions';

  // ===== SIGNALS FOR STATE MANAGEMENT =====

  private returnActionsListSignal = signal<ReturnAction[]>([]);
  private loadingSignal = signal<boolean>(false);
  private errorSignal = signal<string | null>(null);
  private permissionErrorSignal = signal<boolean>(false);
  private lastErrorStatusSignal = signal<number | null>(null);
  private selectedReturnActionSignal = signal<ReturnAction | null>(null);
  private currentPageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);
  private totalReturnActionSignal = signal<number>(0);

  // ===== PUBLIC READONLY SIGNALS =====

  readonly returnActionsList = this.returnActionsListSignal.asReadonly();
  ...
  readonly selectedReturnAction = this.selectedReturnActionSignal.asReadonly();
  readonly currentPage = this.currentPageSignal.asReadonly();
  readonly totalReturnAction = this.totalReturnActionSignal.asReadonly();
  readonly pageSize = this.pageSizeSignal.asReadonly();

  // ===== COMPUTED SIGNALS =====

  readonly totalPages = computed(() => {
    const total = this.totalReturnActionSignal();
  ...
  /**
   * Load returnActions list with pagination and filters
   */
  async loadReturnActionList(params?: ListReturnActionQuery): Promise<void> {
    ...
    try {
      ...
      if (params?.action_code)
        httpParams = httpParams.set('action_code', params.action_code);
      // String filtering for action_name
      if (params?.action_name)
        httpParams = httpParams.set('action_name', params.action_name);
      // Boolean filtering for requires_vendor_approval
      if (params?.requires_vendor_approval !== undefined)
        httpParams = httpParams.set(
          'requires_vendor_approval',
          params.requires_vendor_approval.toString(),
        );
      ...
      const response = await this.http
        .get<
          PaginatedResponse<ReturnAction>
        >(this.baseUrl, { params: httpParams })
        .toPromise();

      if (response) {
        this.returnActionsListSignal.set(response.data);

        if (response.pagination) {
          this.totalReturnActionSignal.set(response.pagination.total);
          this.currentPageSignal.set(response.pagination.page);
          this.pageSizeSignal.set(response.pagination.limit);
        }
      }
    } catch (error: any) {
      this.handleError(error, 'Failed to load returnActions list');
    } finally {
  ...
  /**
   * Load single returnActions by ID
   */
  async loadReturnActionById(id: number): Promise<ReturnAction | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .get<ApiResponse<ReturnAction>>(`${this.baseUrl}/${id}`)
        .toPromise();

      if (response) {
        this.selectedReturnActionSignal.set(response.data);
        return response.data;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to load returnActions');
      return null;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async createReturnAction(
    data: CreateReturnActionRequest,
  ): Promise<ReturnAction | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .post<ApiResponse<ReturnAction>>(`${this.baseUrl}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to create returnActions');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
   ...
   */
  async updateReturnAction(
    id: number,
    data: UpdateReturnActionRequest,
  ): Promise<ReturnAction | null> {
    this.loadingSignal.set(true);

    try {
      const response = await this.http
        .put<ApiResponse<ReturnAction>>(`${this.baseUrl}/${id}`, data)
        .toPromise();
      ...
      this.handleError(error, 'Failed to update returnActions');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Delete returnActions by ID
   */
  async deleteReturnAction(id: number): Promise<boolean> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to delete returnActions');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Export returnActions data
   */
  async exportReturnAction(options: {
    ...
    try {
      ...
      console.error('Failed to export returnActions data:', error);
      throw error;
    }
  }
  ...
  /**
   * Get dropdown options for returnActions
   */
  async getDropdownOptions(
    ...
    try {
      ...
      console.error('Failed to fetch returnActions dropdown options:', error);
      return [];
    }
  }

  /**
   * Bulk create returnActionss
   */
  async bulkCreateReturnAction(
    items: CreateReturnActionRequest[],
    ...
    try {
      ...
      if (response) {
        ...
        await this.loadReturnActionList();
        return response;
      }
      return null;
    } catch (error: any) {
      this.handleError(error, 'Failed to bulk create returnActionss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk update returnActionss
   */
  async bulkUpdateReturnAction(
    items: Array<{ id: number; data: UpdateReturnActionRequest }>,
  ): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk update returnActionss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Bulk delete returnActionss
   */
  async bulkDeleteReturnAction(ids: number[]): Promise<BulkResponse | null> {
    this.loadingSignal.set(true);
    ...
    try {
      ...
      this.handleError(error, 'Failed to bulk delete returnActionss');
      throw error;
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  /**
   * Validate returnActions data before save
   */
  async validateReturnAction(
    data: CreateReturnActionRequest,
    ...
    try {
      ...
      console.error('Failed to validate returnActions:', error);
  ...
  async getStats(): Promise<{ total: number } | null> {
    try {
      ...
      console.error('Failed to get returnActions stats:', error);
      return null;
    }
  }
  ...
  /**
   * Select returnActions
   */
  selectReturnAction(returnActions: ReturnAction | null): void {
    this.selectedReturnActionSignal.set(returnActions);
  }
   ...
   */
  reset(): void {
    this.returnActionsListSignal.set([]);
    this.selectedReturnActionSignal.set(null);
    this.currentPageSignal.set(1);
    this.errorSignal.set(null);
    this.clearPermissionError();
    this.totalReturnActionSignal.set(0);
  }
}
