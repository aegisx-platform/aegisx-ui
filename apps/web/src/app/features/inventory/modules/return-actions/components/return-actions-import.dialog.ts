...
@Component({
  selector: 'app-return-actions-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Return Actions</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="action_code">
                      <th mat-header-cell *matHeaderCellDef>Action Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.action_code }}
                      </td>
                    </ng-container>
        ...
        @if (currentStep() === 'progress' && importJob()) {
          <div class="step-content">
            <div class="progress-container">
              ...
              <h3>Importing Return Actions...</h3>
...
export class ReturnActionImportDialogComponent implements OnDestroy {
  private returnActionsService = inject(ReturnActionService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<ReturnActionImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.returnActionsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `return-actions-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.returnActionsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.returnActionsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.returnActionsService.getImportStatus(jobId);
