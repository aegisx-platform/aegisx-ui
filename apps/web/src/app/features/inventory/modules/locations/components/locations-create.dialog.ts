...
import { CreateLocationRequest } from '../types/locations.types';
import {
  ...
  template: `
    ...
    <h2 mat-dialog-title class="ax-header ax-header-info">
      ...
      <div class="header-text">
        <div class="ax-title">Create New Location</div>
        <div class="ax-subtitle">Add a new location to your collection</div>
      </div>
      <button
        type="button"
        mat-icon-button
...
export class LocationCreateDialogComponent {
  ...
  private dialogRef = inject(MatDialogRef<LocationCreateDialogComponent>);
  ...
  async onFormSubmit(formData: LocationFormData) {
    ...
    try {
      ...
      const createRequest = formData as unknown as CreateLocationRequest;
      const result = await this.locationsService.createLocation(createRequest);
      ...
      if (result) {
        ...
        this.snackBar.open('Failed to create location', 'Close', {
          duration: 5000,
        });
      }
    } catch (error: any) {
      const errorMessage = this.locationsService.permissionError()
        ? 'You do not have permission to create location'
