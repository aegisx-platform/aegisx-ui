...
@Component({
  selector: 'app-locations-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Locations</h2>
...
export class LocationImportDialogComponent implements OnDestroy {
  private locationsService = inject(LocationService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<LocationImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.locationsService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `locations-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.locationsService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.locationsService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.locationsService.getImportStatus(jobId);
