...
@Component({
  selector: 'app-budget-types-import-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Budget Types</h2>
              ...
              <div class="preview-section">
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="budget_class">
                      <th mat-header-cell *matHeaderCellDef>Budget Class</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.budget_class }}
                      </td>
                    </ng-container>
...
export class BudgetTypeImportDialogComponent implements OnDestroy {
  private budgetTypesService = inject(BudgetTypeService);
  private snackBar = inject(MatSnackBar);
  private dialogRef = inject(MatDialogRef<BudgetTypeImportDialogComponent>);
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.budgetTypesService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `budget-types-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.budgetTypesService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.budgetTypesService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.budgetTypesService.getImportStatus(jobId);
