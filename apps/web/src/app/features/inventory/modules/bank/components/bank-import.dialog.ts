    ...
    <div class="tremor-dialog-container">
      ...
      <div class="tremor-dialog-header bg-green-50">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="tremor-dialog-title">Bulk Import Bank</h2>
                ...
                <div class="table-container">
                  ...
                  >
                    ...
                    <ng-container matColumnDef="swift_code">
                      <th mat-header-cell *matHeaderCellDef>Swift Code</th>
                      <td mat-cell *matCellDef="let row">
                        {{ row.swift_code }}
                      </td>
                    </ng-container>
...
export class BankImportDialogComponent implements OnDestroy {
  ...
  downloadTemplate(format: 'csv' | 'excel'): void {
    this.bankService.downloadImportTemplate(format, true).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `bank-import-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
        link.click();
  ...
  async validateFile(): Promise<void> {
    ...
    try {
      const response = await this.bankService.validateImportFile(file);
  ...
  async executeImport(): Promise<void> {
    ...
    try {
      const response = await this.bankService.executeImport(
  ...
  private startPolling(jobId: string): void {
    this.pollingInterval = setInterval(async () => {
      try {
        const response = await this.bankService.getImportStatus(jobId);
