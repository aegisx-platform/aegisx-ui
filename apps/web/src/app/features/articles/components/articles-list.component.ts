...
import { Article, ListArticleQuery } from '../types/articles.types';
import { ArticleCreateDialogComponent } from './articles-create.dialog';
import {
  ArticleEditDialogComponent,
  ArticleEditDialogData,
} from './articles-edit.dialog';
import {
  ArticleViewDialogComponent,
  ArticleViewDialogData,
} from './articles-view.dialog';
...
@Component({
  ...
  template: `
    <div class="articles-list-container">
      ...
      <!-- Data Table -->
      @if (!articlesService.loading() && !articlesService.error()) {
        <mat-card class="table-card">
          <mat-card-content>
            ...
            <div class="table-container">
              <table
                mat-table
                [dataSource]="articlesService.articlesList()"
              ...
              >
                ...
                <ng-container matColumnDef="select">
                  ...
                  <td mat-cell *matCellDef="let articles">
                    <mat-checkbox
            ...
            <!-- Empty State -->
            @if (articlesService.articlesList().length === 0) {
              <div class="empty-state">
                <mat-icon class="empty-icon">inbox</mat-icon>
                <h3>No Articles found</h3>
                <p>Create your first Articles to get started</p>
                <button
                  mat-raised-button
                  color="primary"
                  (click)="openCreateDialog()"
            ...
            @if (articlesService.articlesList().length > 0) {
              <mat-paginator
                [length]="articlesService.totalArticle()"
                [pageSize]="articlesService.pageSize()"
                [pageSizeOptions]="[5, 10, 25, 50, 100]"
                [pageIndex]="articlesService.currentPage() - 1"
...
export class ArticleListComponent implements OnInit, OnDestroy {
  protected articlesService = inject(ArticleService);
  ...
  private filtersSignal = signal<Partial<ListArticleQuery>>({});
  ...
  async loadArticles() {
    const params: ListArticleQuery = {
      page: this.articlesService.currentPage(),
      limit: this.articlesService.pageSize(),
      ...this.filters(),
    };
    ...
    await this.articlesService.loadArticleList(params);
  }

  async retry() {
    this.articlesService.clearError();
  ...
  onSearchChange() {
    ...
    this.searchTimeout = setTimeout(() => {
      this.articlesService.setCurrentPage(1);
      this.loadArticles();
    }, 300);
  }
  ...
  onPageChange(event: PageEvent) {
    this.articlesService.setCurrentPage(event.pageIndex + 1);
    this.articlesService.setPageSize(event.pageSize);
    this.loadArticles();
  }
  ...
  isAllSelected(): boolean {
    const total = this.articlesService.articlesList().length;
  ...
  toggleSelectAll() {
    if (this.isAllSelected()) {
      ...
      const allIds = this.articlesService.articlesList().map((item) => item.id);
  ...
  openCreateDialog() {
    const dialogRef = this.dialog.open(ArticleCreateDialogComponent, {
      width: '600px',
      maxWidth: '90vw',
      disableClose: true,
    });
  ...
  openEditDialog(articles: Article) {
    const dialogRef = this.dialog.open(ArticleEditDialogComponent, {
      width: '600px',
      maxWidth: '90vw',
      disableClose: true,
      data: { articles } as ArticleEditDialogData,
    });
  ...
  openViewDialog(articles: Article) {
    const dialogRef = this.dialog.open(ArticleViewDialogComponent, {
      width: '700px',
      maxWidth: '90vw',
      data: { articles } as ArticleViewDialogData,
    });
  ...
  async deleteArticle(articles: Article) {
    if (confirm(`Are you sure you want to delete this articles?`)) {
      try {
        await this.articlesService.deleteArticle(articles.id);
        this.snackBar.open('Articles deleted successfully', 'Close', {
          duration: 3000,
        });
      } catch (error) {
