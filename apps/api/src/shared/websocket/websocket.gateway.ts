...
export class WebSocketManager {
  ...
  constructor(fastify: FastifyInstance, io: Server) {
    ...
    // WebSocket Manager initialized (silent)
  }
  ...
  private setupEventBusSubscriptions(): void {
    ...
    try {
      ...
      // Auto-forwarding enabled (silent)
    } catch (error) {
      console.error(
        '[WebSocketManager] Failed to setup EventBus subscriptions:',
        error,
      );
    }
  }
  ...
  private async processEventPayload(payload: EventPayload): Promise<void> {
    try {
      ...
      console.error(
        '[WebSocketManager] Error processing event payload:',
        error,
      );
    }
  }
  ...
  private emitToRooms(
    ...
    console.log(
      `📡 [Priority:${message.meta.priority}] Emitted ${eventName} to rooms: ${featureRoom}, ${entityRoom}`,
    );
  }
  ...
  private handleConnection(client: Socket) {
    ...
    if (token) {
      try {
        ...
        console.log(
          `✅ User ${userId} authenticated on connection with priority: ${userPriority}`,
        );
      } catch (error) {
        console.error(
  ...
  private setupActivityMonitoring(client: Socket): void {
    ...
    const healthInterval = setInterval(() => {
      ...
      if (timeSinceActivity > timeoutThreshold) {
        console.warn(
          `⚠️ Inactive client detected: ${client.id}, disconnecting`,
        );
  ...
  getStats() {
    ...
    const activityMetrics = {
      activeLastMinute: clientsArray.filter(
        (c) => now - c.lastActivity.getTime() < 60000,
      ).length,
      activeLastHour: clientsArray.filter(
        (c) => now - c.lastActivity.getTime() < 3600000,
      ).length,
      averageSessionDuration:
        clientsArray.length > 0
          ? clientsArray.reduce(
              (sum, c) => sum + (now - c.joinedAt.getTime()),
              0,
            ) / clientsArray.length
          : 0,
    };
  ...
  private getHealthRecommendations(stats: any): string[] {
    ...
    if (stats.queues.totalQueued > 50) {
      recommendations.push(
        'High queue backlog detected, check processing performance',
      );
    }
    ...
    if (stats.activity.activeLastMinute < stats.connections.total * 0.1) {
      recommendations.push(
        'Low client activity detected, consider connection cleanup',
      );
    }
  ...
  getQueueStatus() {
    return {
      priorities: Array.from(this.priorityQueues.entries()).map(
        ([priority, queue]) => ({
          priority,
          length: queue.length,
