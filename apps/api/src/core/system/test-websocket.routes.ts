...
export default async function testWebSocketRoutes(fastify: FastifyInstance) {
  // ⚠️ SECURITY: Only enable test endpoints in development/test environments
  const isDevelopment =
    process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test';

  // Middleware to block test endpoints in production
  const checkDevelopmentMode = async (request: any, reply: any) => {
    if (!isDevelopment) {
      return reply.code(403).send({
        ...
        error: {
          code: 'FORBIDDEN',
          message:
            'Test endpoints are only available in development/test environments',
        },
      });
    }
  };
  ...
  fastify.post(
    '/test/websocket/emit',
    {
      preHandler: [checkDevelopmentMode],
    },
    async (request, reply) => {
      const {
        feature = 'rbac',
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (fastify.eventService) {
          fastify.eventService.for('rbac', 'role').emitCreated(role);
          ...
          fastify.eventService
            .for('rbac', 'permission')
            .emitCustom('assigned', permissionData);
          ...
          fastify.eventService
            .for('rbac', 'hierarchy')
            .emitCustom('changed', hierarchyData, 'high');
