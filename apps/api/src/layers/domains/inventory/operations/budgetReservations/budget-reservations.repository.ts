...
} from './budget-reservations.types';
...
> {
  ...
  getJoinQuery() {
    return this.knex('inventory.budget_reservations').select(
      'inventory.budget_reservations.*',
    );
    // Add joins here if needed
    // .leftJoin('other_table', 'inventory.budget_reservations.foreign_key', 'other_table.id')
  }
  ...
  protected applyCustomFilters(
    ...
    if (filters.allocation_id !== undefined) {
      query.where(
        'inventory.budget_reservations.allocation_id',
    ...
    if (filters.pr_id !== undefined) {
      query.where('inventory.budget_reservations.pr_id', filters.pr_id);
    }
    if (filters.pr_id_min !== undefined) {
      query.where(
        'inventory.budget_reservations.pr_id',
        '>=',
        filters.pr_id_min,
      );
    }
    ...
    if (filters.reserved_amount !== undefined) {
      query.where(
        'inventory.budget_reservations.reserved_amount',
    ...
    if (filters.quarter !== undefined) {
      query.where('inventory.budget_reservations.quarter', filters.quarter);
    }
    if (filters.quarter_min !== undefined) {
      query.where(
        'inventory.budget_reservations.quarter',
        '>=',
        filters.quarter_min,
      );
    }
    ...
    if (filters.is_released !== undefined) {
      query.where(
        'inventory.budget_reservations.is_released',
        filters.is_released,
      );
    }
  }
  ...
  protected getSortField(sortBy: string): string {
    const sortFields: Record<string, string> = {
      id: 'inventory.budget_reservations.id',
      allocationId: 'inventory.budget_reservations.allocation_id',
      prId: 'inventory.budget_reservations.pr_id',
      reservedAmount: 'inventory.budget_reservations.reserved_amount',
      quarter: 'inventory.budget_reservations.quarter',
      reservationDate: 'inventory.budget_reservations.reservation_date',
      expiresDate: 'inventory.budget_reservations.expires_date',
      isReleased: 'inventory.budget_reservations.is_released',
      releasedAt: 'inventory.budget_reservations.released_at',
      createdAt: 'inventory.budget_reservations.created_at',
      updatedAt: 'inventory.budget_reservations.updated_at',
    };

    return sortFields[sortBy] || 'inventory.budget_reservations.id';
  }
  ...
  async getStats(): Promise<{
    ...
    const stats: any = await this.knex('inventory.budget_reservations')
  ...
  async createMany(
    ...
    const rows = await this.knex('inventory.budget_reservations')
  ...
  async createWithTransaction(
    ...
    return this.withTransaction(async (trx) => {
      ...
      const [row] = await trx('inventory.budget_reservations')
        .insert(transformedData)
        .returning('*');
