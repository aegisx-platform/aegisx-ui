...
> {
  constructor(knex: Knex) {
    super(
      knex,
      'inventory.budget_plans',
      [
        // Define searchable fields based on intelligent detection
        'inventory.budget_plans.plan_name',
      ],
      [], // explicitUUIDFields
      {
  ...
  getJoinQuery() {
    return this.knex('inventory.budget_plans').select(
      'inventory.budget_plans.*',
    );
    // Add joins here if needed
    // .leftJoin('other_table', 'inventory.budget_plans.foreign_key', 'other_table.id')
  }
  ...
  protected applyCustomFilters(
    ...
    if (filters.fiscal_year !== undefined) {
      query.where('inventory.budget_plans.fiscal_year', filters.fiscal_year);
    }
    if (filters.fiscal_year_min !== undefined) {
      query.where(
        'inventory.budget_plans.fiscal_year',
        '>=',
    ...
    if (filters.department_id !== undefined) {
      query.where(
        'inventory.budget_plans.department_id',
    ...
    if (filters.plan_name !== undefined) {
      query.where('inventory.budget_plans.plan_name', filters.plan_name);
    }
    if (filters.total_planned_amount !== undefined) {
      query.where(
        'inventory.budget_plans.total_planned_amount',
    ...
    if (filters.approved_by !== undefined) {
      query.where('inventory.budget_plans.approved_by', filters.approved_by);
    }
    if (filters.is_active !== undefined) {
      query.where('inventory.budget_plans.is_active', filters.is_active);
    }
  }
  ...
  protected getSortField(sortBy: string): string {
    const sortFields: Record<string, string> = {
      id: 'inventory.budget_plans.id',
      fiscalYear: 'inventory.budget_plans.fiscal_year',
      departmentId: 'inventory.budget_plans.department_id',
      planName: 'inventory.budget_plans.plan_name',
      totalPlannedAmount: 'inventory.budget_plans.total_planned_amount',
      status: 'inventory.budget_plans.status',
      approvedAt: 'inventory.budget_plans.approved_at',
      approvedBy: 'inventory.budget_plans.approved_by',
      isActive: 'inventory.budget_plans.is_active',
      createdAt: 'inventory.budget_plans.created_at',
      updatedAt: 'inventory.budget_plans.updated_at',
    };

    return sortFields[sortBy] || 'inventory.budget_plans.id';
  }
  ...
  async getStats(): Promise<{
    ...
    const stats: any = await this.knex('inventory.budget_plans')
  ...
  async createMany(data: CreateBudgetPlans[]): Promise<BudgetPlans[]> {
    ...
    const rows = await this.knex('inventory.budget_plans')
  ...
  async createWithTransaction(data: CreateBudgetPlans): Promise<BudgetPlans> {
    return this.withTransaction(async (trx) => {
      ...
      const [row] = await trx('inventory.budget_plans')
        .insert(transformedData)
        .returning('*');
