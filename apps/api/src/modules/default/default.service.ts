...
export class DefaultService {
  ...
  async getSystemStatus(): Promise<SystemStatus> {
    ...
    const redisStatus = this.fastify?.redis ? await this.checkRedis() : undefined;
    
    // Get memory status
    const memoryStatus = this.getMemoryStatus();
    
    // Determine overall status
    const overallStatus = this.determineOverallStatus(databaseStatus, redisStatus, memoryStatus);
  ...
  private determineOverallStatus(
    ...
    if (databaseStatus.status === 'error' || databaseStatus.status === 'disconnected') {
      return 'unhealthy';
    }
    ...
    if (redisStatus && (redisStatus.status === 'error' || redisStatus.status === 'disconnected')) {
      return 'degraded';
    }
