...
> {
  ...
  getJoinQuery() {
    return this.knex('drug_returns').select('drug_returns.*');
    // Add joins here if needed
    // .leftJoin('other_table', 'drug_returns.foreign_key', 'other_table.id')
  }
  ...
  protected applyCustomFilters(
    ...
    if (filters.return_number !== undefined) {
      query.where('drug_returns.return_number', filters.return_number);
    }
    if (filters.department_id !== undefined) {
      query.where('drug_returns.department_id', filters.department_id);
    }
    if (filters.department_id_min !== undefined) {
      query.where(
        'drug_returns.department_id',
        '>=',
    ...
    if (filters.return_reason_id !== undefined) {
      query.where('drug_returns.return_reason_id', filters.return_reason_id);
    }
    if (filters.return_reason_id_min !== undefined) {
      query.where(
        'drug_returns.return_reason_id',
        '>=',
    ...
    if (filters.return_reason !== undefined) {
      query.where('drug_returns.return_reason', filters.return_reason);
    }
    if (filters.action_taken !== undefined) {
      query.where('drug_returns.action_taken', filters.action_taken);
    }
    if (filters.total_items !== undefined) {
      query.where('drug_returns.total_items', filters.total_items);
    }
    if (filters.total_items_min !== undefined) {
      query.where('drug_returns.total_items', '>=', filters.total_items_min);
    }
    if (filters.total_items_max !== undefined) {
      query.where('drug_returns.total_items', '<=', filters.total_items_max);
    }
    if (filters.total_amount !== undefined) {
      query.where('drug_returns.total_amount', filters.total_amount);
    }
    if (filters.total_amount_min !== undefined) {
      query.where('drug_returns.total_amount', '>=', filters.total_amount_min);
    }
    if (filters.total_amount_max !== undefined) {
      query.where('drug_returns.total_amount', '<=', filters.total_amount_max);
    }
    if (filters.received_by !== undefined) {
      query.where('drug_returns.received_by', filters.received_by);
    }
    if (filters.verified_by !== undefined) {
      query.where('drug_returns.verified_by', filters.verified_by);
    }
    if (filters.notes !== undefined) {
      query.where('drug_returns.notes', filters.notes);
    }
  }
  ...
  protected getSortField(sortBy: string): string {
    const sortFields: Record<string, string> = {
      id: 'drug_returns.id',
      returnNumber: 'drug_returns.return_number',
      departmentId: 'drug_returns.department_id',
      returnDate: 'drug_returns.return_date',
      returnReasonId: 'drug_returns.return_reason_id',
      returnReason: 'drug_returns.return_reason',
      actionTaken: 'drug_returns.action_taken',
      status: 'drug_returns.status',
      totalItems: 'drug_returns.total_items',
      totalAmount: 'drug_returns.total_amount',
      receivedBy: 'drug_returns.received_by',
      verifiedBy: 'drug_returns.verified_by',
      notes: 'drug_returns.notes',
      createdAt: 'drug_returns.created_at',
      updatedAt: 'drug_returns.updated_at',
    };

    return sortFields[sortBy] || 'drug_returns.id';
  }
  ...
  async getStats(): Promise<{
    ...
    const stats: any = await this.knex('drug_returns')
  ...
  async createMany(data: CreateDrugReturns[]): Promise<DrugReturns[]> {
    ...
    const rows = await this.knex('drug_returns')
  ...
  async createWithTransaction(data: CreateDrugReturns): Promise<DrugReturns> {
    return this.withTransaction(async (trx) => {
      ...
      const [row] = await trx('drug_returns')
        .insert(transformedData)
        .returning('*');
