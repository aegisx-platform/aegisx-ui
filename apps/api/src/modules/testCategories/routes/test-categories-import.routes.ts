...
export interface TestCategoriesImportRoutesOptions {
  controller: TestCategoriesController;
}

/**
 * Register testCategories import routes
 */
export async function testCategoriesImportRoutes(
  fastify: FastifyInstance,
  options: TestCategoriesImportRoutesOptions,
) {
  ...
  fastify.get('/import/template', {
    schema: {
      tags: ['TestCategories', 'Import'],
      summary: 'Download import template',
      description:
        'Download Excel/CSV template for testCategories import with field instructions and example data',
      querystring: {
        type: 'object',
        properties: {
          format: {
  ...
  fastify.post('/import/execute', {
    schema: {
      ...
      description:
        'Execute validated import using session ID from validate endpoint. Creates/updates testCategories in database.',
