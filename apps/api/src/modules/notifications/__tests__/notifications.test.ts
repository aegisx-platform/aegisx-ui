...
import { NotificationsCreate, NotificationsUpdate } from '../schemas/notifications.types';
 ...
 * 
 * Generated on: 2025-10-06T07:55:28.592Z
 */
...
describe('Notifications Domain', () => {
  ...
  describe('Notifications Routes', () => {
    let createdNotificationsId: string;

    // Test data factory
    const createNotificationsData = (): NotificationsCreate => ({
    ...
    describe('POST /core', () => {
      it('should create a new core', async () => {
        ...
        expect(body.data).toMatchObject(notificationsData);
        expect(body.data.id).toBeDefined();
        expect(body.data.created_at).toBeDefined();
        expect(body.data.updated_at).toBeDefined();

        // Store ID for other tests
        createdNotificationsId = body.data.id;
      });
    ...
    describe('GET /core/:id', () => {
      it('should get core by ID', async () => {
        const response = await app.inject({
          method: 'GET',
          url: `/core/${createdNotificationsId}`
        });
        ...
        const body = response.json();
        expect(body.success).toBe(true);
        expect(body.data.id).toBe(createdNotificationsId);
      });
    ...
    describe('PUT /core/:id', () => {
      it('should update core', async () => {
        const updateData: NotificationsUpdate = {
          name: 'Updated Notifications Name',
    ...
    describe('DELETE /core/:id', () => {
      ...
      it('should return 404 for non-existent core', async () => {
        const response = await app.inject({
          method: 'DELETE',
          url: `/core/${createdNotificationsId}` // Try to delete again
        });
