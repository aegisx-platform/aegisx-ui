...
export class SettingsQueryOptimizer {
  ...
  static async getGroupedSettingsOptimized(
    ...
    const settings = await knex('app_settings')
      ...
      .orderBy('sort_order')
      .select(
        'id', 'key', 'value', 'label', 'description',
        'category', 'group', 'data_type', 'access_level'
      );
   ...
   */
  static async analyzeQuery(knex: Knex, query: Knex.QueryBuilder): Promise<any> {
    const sql = query.toSQL();
    const result = await knex.raw(`EXPLAIN (ANALYZE, BUFFERS) ${sql.sql}`, sql.bindings);
    return result.rows;
  }
}
...
export class SettingsCacheWarmer {
   ...
   */
  static async warmFrequentSettings(
    knex: Knex,
    redis: any
  ): Promise<void> {
  ...
  static async warmUserSettings(
    ...
    // Group by user
    const settingsByUser = userSettings.reduce((acc, setting) => {
