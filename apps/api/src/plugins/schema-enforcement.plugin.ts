...
export default fp(
  async function schemaEnforcementPlugin(fastify: FastifyInstance) {
    ...
    fastify.addHook('onRoute', (routeOptions) => {
      ...
      if (routeOptions.url === '/health' || routeOptions.url.startsWith('/.well-known/') || routeOptions.url.includes('*')) {
        return;
      }

      // Skip Swagger UI routes
      if (routeOptions.url.includes('/documentation') || routeOptions.url.includes('/api-docs') || routeOptions.url.includes('/swagger')) {
        return;
      }

      // Skip existing auth and user-profile routes temporarily (they need to be updated separately)
      if (routeOptions.url.includes('/auth/') || routeOptions.url.includes('/users/')) {
        return;
      }
      ...
      const methodArray = Array.isArray(routeOptions.method) ? routeOptions.method : [routeOptions.method];
      const hasPostPutPatch = methodArray.some(method => ['POST', 'PUT', 'PATCH'].includes(method));
      ...
      if (!routeOptions.schema.tags || routeOptions.schema.tags.length === 0) {
        throw new Error(`Route ${routeOptions.method} ${routeOptions.url} must have tags for Swagger documentation`);
      }

      // Ensure all routes have description
      if (!routeOptions.schema.description) {
        throw new Error(`Route ${routeOptions.method} ${routeOptions.url} must have description`);
      }

      // Ensure all routes have summary
      if (!routeOptions.schema.summary) {
        throw new Error(`Route ${routeOptions.method} ${routeOptions.url} must have summary`);
      }
    });
    ...
    fastify.log.info('Schema enforcement plugin activated - all routes must have complete schemas');
  },
  {
