...
async function activityLoggingPlugin(
  ...
  fastify.addHook('onReady', async function () {
    if (!fastify.knex) {
      throw new Error(
        'Activity logging plugin requires knex plugin to be registered first',
      );
    }
  ...
  fastify.decorate(
    ...
    async function (
      ...
      if (!userActivityService) {
        fastify.log.warn(
          'User activity service not initialized, skipping manual activity log',
        );
        return;
      }
  ...
  fastify.decorate(
    'withActivityLogging',
    function (routeConfig: ActivityLogConfig, routeHandler: any) {
      return {
        ...routeHandler,
        schema: {
          ...routeHandler.schema,
...
declare module 'fastify' {
  interface FastifyInstance {
    ...
    withActivityLogging(routeConfig: ActivityLogConfig, routeHandler: any): any;
