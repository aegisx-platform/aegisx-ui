...
describe('User Profile API Integration Tests', () => {
  ...
  const invalidImagePath = path.join(__dirname, 'test-assets', 'test-document.txt');
  ...
  beforeAll(async () => {
    ...
    // Create test users
    const testUserResult = await authHelper.createUserWithRole('user', [
      'profile.read',
      'profile.write'
    ], {
    ...
    const testUserTokens = await authHelper.loginUser(testUser.email, testUser.password);
    ...
    const otherUserResult = await authHelper.createUserWithRole('user', [
      'profile.read',
      'profile.write'
    ], {
    ...
    const otherUserTokens = await authHelper.loginUser(otherUser.email, otherUser.password);
  ...
  describe('PUT /api/users/profile', () => {
    ...
    describe('Profile Updates', () => {
      ...
      it('should not allow email updates through profile endpoint', async () => {
        ...
        const user = await testContext.db.connection('users')
          .where('id', testUser.id)
          .first();
  ...
  describe('PUT /api/users/preferences', () => {
    it('should update user preferences', async () => {
      ...
      // Verify in database
      const prefs = await testContext.db.connection('user_preferences')
        .where('user_id', testUser.id)
        .first();
